...
class ETACalculator {
  ...
  ETAResult calculateETA({
    ...
    final remainingDistance =
        _calculateRemainingDistance(route, currentLatitude, currentLongitude);
    ...
    if (currentPace != null && currentPace > 0) {
      final currentETA =
          Duration(seconds: (remainingDistance / currentPace).round());
      etas['current_pace'] = currentETA;
      confidences['current_pace'] =
          _calculatePaceConfidence(currentPace, recentSpeeds);
    }

    // Method 2: Average Pace ETA
    if (averagePace != null && averagePace > 0) {
      final averageETA =
          Duration(seconds: (remainingDistance / averagePace).round());
      etas['average_pace'] = averageETA;
      confidences['average_pace'] =
          _calculateAverageConfidence(averagePace, sessionDuration);
    }
    ...
    if (recentSpeeds.isNotEmpty) {
      ...
      final movingETA =
          Duration(seconds: (remainingDistance / movingAverage).round());
      etas['moving_average'] = movingETA;
      confidences['moving_average'] =
          _calculateMovingAverageConfidence(recentSpeeds);
    }
    ...
    confidences['terrain_adjusted'] =
        0.7; // Moderate confidence for terrain estimates
  ...
  double _calculateRemainingDistance(
      Route route, double currentLat, double currentLon) {
  ...
  Duration _calculateTerrainAdjustedETA(
    ...
    final remainingDistance =
        _calculateRemainingDistance(route, currentLat, currentLon);
    ...
    double baseSpeed = targetWeight != null && targetWeight > 0
        ? _averageRuckingSpeed *
            (1 - (targetWeight / 100) * 0.1) // 10% slower per 100lbs
        : _averageWalkingSpeed;

    // Calculate elevation changes in remaining route
    final elevationAdjustment =
        _calculateElevationAdjustment(route, currentLat, currentLon);
  ...
  double _calculateMovingAverage(List<double> recentSpeeds) {
    ...
    for (int i = 0; i < recentSpeeds.length; i++) {
      final weight =
          i / recentSpeeds.length + 0.5; // More recent = higher weight
    ...
    return math.max(
        _minReasonableSpeed, math.min(_maxReasonableSpeed, average));
  }
  ...
  Duration _calculateAdaptiveETA(
    ...
    etas.forEach((method, eta) {
      if (method != 'adaptive') {
        // Don't include adaptive in its own calculation
        final confidence = confidences[method] ?? 0.5;
        final weight =
            confidence * confidence; // Square for more pronounced weighting
  ...
  double _calculatePaceConfidence(
      double currentPace, List<double> recentSpeeds) {
    ...
    final consistencyScore =
        math.max(0.0, 1.0 - (deviation / (standardDeviation + 0.1)));
  ...
  double _calculateAverageConfidence(
      double averagePace, Duration? sessionDuration) {
    ...
    final timeFactor = (timeFactorMinutes / 30)
        .clamp(0.0, 1.0); // Max confidence at 30+ minutes
