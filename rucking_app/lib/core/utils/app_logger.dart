...
class AppLogger {
  ...
  static const bool _verboseLogs =
  ...
  static void critical(String message,
    ...
    try {
      ...
      debugPrint(
          '$_errorPrefix Failed to send critical log to Crashlytics: $e');
    }
  }
  ...
  static void sessionCompletion(String step, {Map<String, dynamic>? context}) {
    ...
    try {
      ...
      FirebaseCrashlytics.instance
          .setCustomKey('session_completion_step', step);
      FirebaseCrashlytics.instance.setCustomKey(
          'session_completion_time', DateTime.now().toIso8601String());
    } catch (e) {
      debugPrint(
          '$_errorPrefix Failed to send session completion log to Crashlytics: $e');
    }
  }
  ...
  static void sessionTimeout(
    ...
    try {
      ...
      if (sessionId != null) {
        FirebaseCrashlytics.instance
            .setCustomKey('timeout_session_id', sessionId);
      }
      if (duration != null) {
        FirebaseCrashlytics.instance
            .setCustomKey('timeout_duration_seconds', duration);
      }
      if (lastStep != null) {
        FirebaseCrashlytics.instance
            .setCustomKey('timeout_last_step', lastStep);
      }
      if (networkInfo != null) {
        networkInfo.forEach((key, value) {
          FirebaseCrashlytics.instance
              .setCustomKey('timeout_$key', value.toString());
        });
      }

      FirebaseCrashlytics.instance
          .setCustomKey('timeout_timestamp', DateTime.now().toIso8601String());
  ...
  static bool _shouldSuppress(String message) {
    ...
    if (_isAiDebug(message))
      return false; // never suppress explicit AI debug logs
