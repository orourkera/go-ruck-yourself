...
class PushNotificationTest {
  ...
  static Future<Map<String, dynamic>> runDiagnostics() async {
    ...
    try {
      ...
      try {
        ...
        results['permission_request_status'] =
            requestedSettings.authorizationStatus.toString();
        AppLogger.info(
            '🔔 Permission Request Result: ${requestedSettings.authorizationStatus}');
      } catch (e) {
  ...
  static Future<void> testSetup() async {
    ...
    if (diagnostics['permission_status'] != 'AuthorizationStatus.authorized') {
      ...
      AppLogger.info(
          '💡 FIX: Go to Settings > Notifications > Your App > Allow Notifications');
    }
    ...
    AppLogger.info(
        '   Expected APS Environment: ${diagnostics['expected_aps_environment']}');
    AppLogger.info(
        '   Note: Xcode automatically sets aps-environment based on build type');
