...
import 'package:supabase_flutter/supabase_flutter.dart'
    show Supabase, OAuthProvider, AuthResponse, AuthState, AuthChangeEvent;
...
class AuthServiceImpl implements AuthService {
  ...
  static const int _maxRefreshFailures =
      5; // Increased from 3 to 5 to be less aggressive
  ...
  static const Duration _profileCacheDuration =
      Duration(seconds: 30); // Increased from 5 to 30 seconds
  ...
  AuthServiceImpl(this._apiClient, this._storageService) {
    _googleSignIn = GoogleSignIn(
      ...
      clientId: Platform.isIOS
          ...
          : null, // iOS client ID only, Android uses google-services.json
    );
  ...
  Future<User> signIn(String email, String password) async {
    try {
      ...
      await _storageService.setSecureString(
          AppConfig.refreshTokenKey, refreshToken);
      ...
      if (user == null) {
        throw ApiException(
            'Authentication succeeded but could not get user data');
      }
  ...
  Future<User> googleSignIn() async {
    try {
      ...
      if (googleUser == null) {
        throw AuthException(
            'Google Sign-In was cancelled', 'GOOGLE_SIGNIN_CANCELLED');
      }
      ...
      if (accessToken == null) {
        throw AuthException(
            'No Access Token found', 'GOOGLE_AUTH_NO_ACCESS_TOKEN');
      }
      if (idToken == null) {
      ...
      if (supabaseUser == null || session == null) {
        throw AuthException(
            'Supabase authentication failed', 'SUPABASE_AUTH_FAILED');
      }

      AppLogger.info(
          'Google authentication successful, user: ${supabaseUser.email}');

      // Store tokens FIRST before making any API calls to prevent token expiration issues
      await _storageService.setSecureString(
          AppConfig.tokenKey, session.accessToken);
      await _storageService.setSecureString(
          AppConfig.refreshTokenKey, session.refreshToken!);
      ...
      try {
        ...
        AppLogger.info(
            'Google Sign-In successful for existing user: ${user.email}');
        return user;
      } catch (e) {
        AppLogger.info(
            'User profile not found, creating new profile for Google user: ${supabaseUser.email}');

        // User doesn't exist in your backend, create new profile
        final newUserData = {
          'email': supabaseUser.email!,
          'username': supabaseUser.userMetadata?['full_name'] ??
        ...
        try {
          final createResponse =
              await _apiClient.post('/users/profile', newUserData);
          final newUser = User.fromJson(createResponse);

          AppLogger.info(
              'Google Sign-In successful for new user: ${newUser.email}');
          return newUser;
        } catch (createError) {
          AppLogger.error('Failed to create user profile after Google login',
      ...
      throw AuthException(
          'Google Sign-In failed: ${e.toString()}', 'GOOGLE_SIGNIN_ERROR');
    }
  }
  ...
  Future<User> appleSignIn() async {
    try {
      ...
      if (credential.identityToken == null) {
        throw AuthException(
            'No Identity Token found', 'APPLE_AUTH_NO_ID_TOKEN');
      }
      ...
      AppLogger.info(
          'Apple authentication successful, user: ${supabaseUser.email}');
      ...
      try {
        ...
        AppLogger.info(
            'Apple Sign-In successful for existing user: ${user.email}');
        return user;
      } catch (e) {
        AppLogger.info(
            'User profile not found, creating new profile for Apple user: ${supabaseUser.email}');
        ...
        try {
          ...
          AppLogger.info(
              'Apple Sign-In successful for new user: ${newUser.email}');
          return newUser;
        } catch (createError) {
          AppLogger.error('Failed to create user profile after Apple login',
              exception: createError);
          throw AuthException(
      ...
      if (e is SignInWithAppleAuthorizationException) {
        if (e.code == AuthorizationErrorCode.canceled) {
          throw AuthException(
              'Apple Sign-In was cancelled', 'APPLE_SIGNIN_CANCELLED');
        }
        throw AuthException(
            'Apple Sign-In failed: ${e.message}', 'APPLE_SIGNIN_ERROR');
      }
      ...
      throw AuthException(
          'Apple Sign-In failed: ${e.toString()}', 'APPLE_SIGNIN_ERROR');
    }
  }
  ...
  Future<User> googleRegister({
    ...
    try {
      ...
      if (session == null) {
        throw AuthException(
            'No active Google session found', 'GOOGLE_SESSION_MISSING');
      }
  ...
  Future<User> register({
    ...
    try {
      ...
      if (userData == null) {
        throw Exception(
            'Registration failed: No user data returned from server.');
      }
  ...
  Future<bool> isAuthenticated() async {
    ...
    AppLogger.info(
        '[AUTH] Checking authentication - token exists: ${token != null}');
    if (token == null) {
    ...
    try {
      ...
      if (e is UnauthorizedException) {
        ...
        try {
          ...
          AppLogger.info(
              '[AUTH] Token refresh result: ${refreshSuccessful ? 'success' : 'failed'}');
  ...
  Future<String?> refreshToken() async {
    try {
      ...
      if (!hasRunTokenCleanup) {
        ...
        AppLogger.info(
            '[AUTH] Token cleanup completed - user will need to re-authenticate');
      ...
      AppLogger.debug(
          '[AUTH] Making refresh request with token: ${refreshToken.substring(0, 10)}...');
      ...
      if (newToken == null ||
        ...
        AppLogger.error(
            '[AUTH] Invalid refresh response - missing or empty tokens');
      ...
      try {
        ...
        AppLogger.warning(
            '[AUTH] Refreshed token was immediately rejected: $testError');
        await _handleRefreshFailure();
        return null;
      }
    } catch (e) {
      ...
      if (e.toString().contains('401') ||
        ...
        AppLogger.error(
            '[AUTH] Refresh token is invalid or expired - clearing stored tokens');
  ...
  Future<User> _fetchUserProfileWithRetry({bool forceRefresh = false}) async {
    ...
    if (!forceRefresh &&
        ...
        DateTime.now().difference(_lastProfileRequest!) <
  ...
  Future<User> _performProfileRequestWithRetry() async {
    ...
    for (int attempt = 0; attempt < maxRetries; attempt++) {
      try {
        AppLogger.info(
            'Fetching user profile (attempt ${attempt + 1}/$maxRetries)');
        final response = await _apiClient.get('/users/profile');
        final user = User.fromJson(response);

        // Store the fetched profile data
        await _storageService.setObject(
            AppConfig.userProfileKey, user.toJson());
        await _storageService.setString(AppConfig.userIdKey, user.userId);
        ...
        if (isRateLimited && !isLastAttempt) {
          ...
          AppLogger.warning(
              'Rate limited (429), retrying in ${delay.inSeconds}s (attempt ${attempt + 1}/$maxRetries)');
        ...
        AppLogger.error('Profile fetch failed after ${attempt + 1} attempts',
            exception: e);
        throw e;
      }
    }
  ...
  Future<void> _handleRefreshFailure() async {
    if (_consecutiveRefreshFailures >= _maxRefreshFailures) {
      AppLogger.warning(
          '[AUTH] Too many consecutive refresh failures ($_consecutiveRefreshFailures). User may be deleted. Forcing logout.');
      ...
      AppLogger.info(
          '[AUTH] User logged out due to authentication failures. App should redirect to login.');
    }
  }
  ...
  Future<void> _cleanupInvalidAuthState() async {
    AppLogger.warning(
        '[AUTH] Invalid or expired tokens detected, but maintaining user session');
    ...
    AppLogger.info(
        '[AUTH] User session maintained - will attempt to recover on next API call');
  }
  ...
  Future<User> updateProfile({
    ...
    AppLogger.info(
        '[AUTH] 🔧 Update fields: preferMetric=$preferMetric, username=$username, weightKg=$weightKg');
    ...
    try {
      ...
      if (allowRuckSharing != null)
        data['allow_ruck_sharing'] = allowRuckSharing;
      if (gender != null) data['gender'] = gender;
      if (avatarUrl != null) data['avatar_url'] = avatarUrl;
      if (notificationClubs != null)
        data['notification_clubs'] = notificationClubs;
      if (notificationBuddies != null)
        data['notification_buddies'] = notificationBuddies;
      if (notificationEvents != null)
        data['notification_events'] = notificationEvents;
      if (notificationDuels != null)
      ...
      if (calorieActiveOnly != null)
        data['calorie_active_only'] = calorieActiveOnly;

      // Only send request if there is data to update
      if (data.isEmpty) {
        AppLogger.warning(
            '[AUTH] 🔧 No profile data to update, fetching current user');
      ...
      AppLogger.info(
          '[AUTH] 🔧 Authentication status before profile update: $isAuth');
      ...
      for (int attempt = 1; attempt <= maxRetries; attempt++) {
        try {
          ...
          if (e.toString().contains('UnauthorizedException') ||
            ...
            AppLogger.info(
                '[AUTH] 🔧 Auth error detected, waiting before retry...');
            await Future.delayed(
                Duration(milliseconds: 500 * attempt)); // 500ms, then 1s
            ...
            try {
              ...
              if (refreshedToken != null) {
                ...
                AppLogger.warning(
                    '[AUTH] 🔧 Token refresh failed before retry');
              }
            } catch (refreshError) {
              AppLogger.warning(
                  '[AUTH] 🔧 Token refresh error before retry: $refreshError');
            }
          } else {
      ...
      if (e.toString().contains('UnauthorizedException') ||
          e.toString().contains('Not authenticated')) {
        AppLogger.error(
            '[AUTH] 🔧 AUTHENTICATION ERROR DETECTED - this is the bug we\'re tracking!');
        // Log additional debugging info
        final hasToken =
            await _storageService.getSecureString(AppConfig.tokenKey) != null;
        final hasRefreshToken =
            await _storageService.getSecureString(AppConfig.refreshTokenKey) !=
                null;
        AppLogger.error(
            '[AUTH] 🔧 Has token: $hasToken, Has refresh token: $hasRefreshToken');
      }
      throw _handleAuthError(e);
    }
  }
  ...
  Future<void> deleteAccount({required String userId}) async {
    try {
      ...
      AppLogger.info(
          "AuthService: Backend account deletion successful for $userId. Signing out locally.");
      ...
      AppLogger.warning(
          "AuthService: Unauthorized during delete account for $userId. Signing out. Error: $e");
      await signOut();
      ...
      AppLogger.error(
          "AuthService: Error during delete account API call for $userId: $e");
  ...
  Future<void> requestPasswordReset({required String email}) async {
    try {
      ...
      if (response == null) {
        throw Exception(
            'Password reset request failed: No response from server.');
      }
  ...
  Future<void> confirmPasswordReset({
    ...
    try {
      ...
      if (response == null) {
        throw Exception(
            'Password reset confirmation failed: No response from server.');
      }
