...
class BarometerService {
  ...
  final StreamController<BarometricReading> _controller =
      StreamController<BarometricReading>.broadcast();

  // Calibration variables for absolute altitude conversion
  double? _seaLevelPressurePa =
      101325.0; // Standard sea level pressure in Pascals
  ...
  Future<void> startStreaming() async {
    ...
    try {
        ...
        (dynamic data) {
          try {
            if (data is Map<String, dynamic>) {
              ...
              AppLogger.warning(
                  '[BAROMETER] Received invalid data format: ${data.runtimeType}');
            }
          } catch (e) {
            AppLogger.error(
                '[BAROMETER] Error processing barometric reading: $e');
          }
        },
        onError: (dynamic error) {
          ...
          if (error is PlatformException) {
            if (error.code == 'UNAVAILABLE') {
              AppLogger.warning(
                  '[BAROMETER] Barometric sensor not available on this device');
            } else {
      ...
      AppLogger.info(
          '[BAROMETER] Barometric pressure streaming started successfully');
    } catch (e) {
  ...
  void calibrateWithGPS({
    ...
    AppLogger.info(
        '[BAROMETER] Calibrated with GPS - altitude: ${gpsAltitudeM.toStringAsFixed(1)}m, pressure: ${pressurePa.toStringAsFixed(0)}Pa');
  }
  ...
  double pressureToAltitude(double pressurePa) {
    ...
    if (_referencePressurePa != null && _referenceAltitudeM != null) {
      ...
      final altitudeDelta =
          (44330 * (1 - math.pow(pressurePa / _referencePressurePa!, 0.1903)))
              .toDouble();
      return _referenceAltitudeM! + altitudeDelta;
    } else {
      // Fallback to standard sea level pressure
      return (44330 * (1 - math.pow(pressurePa / _seaLevelPressurePa!, 0.1903)))
          .toDouble();
    }
  }
  ...
  void _processBarometricReading(BarometricReading reading) {
    ...
    AppLogger.debug(
        '[BAROMETER] Pressure: ${reading.pressurePa.toStringAsFixed(0)}Pa, '
