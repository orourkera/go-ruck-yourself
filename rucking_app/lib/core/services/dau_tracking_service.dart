...
class DauTrackingService with WidgetsBindingObserver {
  ...
  Future<void> _trackAppOpen() async {
    try {
      ...
      if (lastUpdate == null || now.difference(lastUpdate) > _updateCooldown) {
        AppLogger.info(
            'Updating last_active_at (last update: ${lastUpdate?.toIso8601String() ?? 'never'})');
  ...
  Future<bool> _sendLastActiveUpdate() async {
    try {
      ...
      if (user == null) {
        AppLogger.debug(
            'DAU tracking - No authenticated user, skipping update');
        return false;
      }
      ...
      if (user.userId.isEmpty) {
        AppLogger.warning(
            'DAU tracking - User has empty userId, skipping update');
        return false;
      }
    ...
    try {
      ...
      if (pendingUpdateStr != null) {
        ...
        if (success) {
          ...
          AppLogger.warning(
              'Failed to send pending DAU update - will retry later');
        }
      }
    } catch (e) {
  ...
  Future<void> _waitForAuthenticationReady() async {
    ...
    AppLogger.warning(
        'DAU tracking - Timeout waiting for authentication state');
  }
  ...
  Future<User?> _getValidatedUser() async {
    try {
      ...
      if (currentState is! Authenticated) {
        AppLogger.debug(
            'DAU tracking - AuthBloc state is not Authenticated: ${currentState.runtimeType}');
        return null;
      }
      ...
      if (user == null) {
        AppLogger.warning(
            'DAU tracking - AuthBloc shows authenticated but AuthService returned null');
        return null;
      }
      ...
      if (user.userId != currentState.user.userId) {
        AppLogger.warning(
            'DAU tracking - User ID mismatch between AuthBloc and AuthService');
        return null;
      }
  ...
  Future<bool> _handleApiError(dynamic error) async {
    if (error is UnauthorizedException ||
      ...
      try {
        ...
        if (refreshedToken != null) {
          AppLogger.info(
              'DAU tracking - Token refreshed successfully, retrying operation');
      ...
      try {
        final authBloc = GetIt.instance<AuthBloc>();
        AppLogger.info(
            'DAU tracking - Triggering authentication check due to auth error');
    ...
    if (error.toString().contains('ServerException')) {
      AppLogger.error(
          'DAU tracking - Server error (likely auth/permissions): $error');
    } else if (error.toString().contains('TimeoutException')) {
      AppLogger.warning('DAU tracking - Request timeout: $error');
    } else {
      AppLogger.warning(
          'DAU tracking - Failed to update last_active_at: $error');
    }
