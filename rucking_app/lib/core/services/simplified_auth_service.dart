...
class SimplifiedAuthService {
  ...
  Future<User> signInSimplified(String email, String password) async {
    ...
    try {
      ...
      if (response.user == null || response.session == null) {
        throw AuthException(
            'Sign-in failed: No user or session returned', 'SIGNIN_FAILED');
      }
      ...
      AppLogger.info(
          '[SIMPLIFIED_AUTH] 🔐 Supabase sign-in successful for: ${supabaseUser.email}');
      ...
      AppLogger.info(
          '[SIMPLIFIED_AUTH] 🔐 Sign-in complete for user: ${user.userId}');
  ...
  Future<User> signUpSimplified({
    ...
    try {
      ...
      if (response.user == null || response.session == null) {
        throw AuthException(
            'Sign-up failed: No user or session returned', 'SIGNUP_FAILED');
      }
      ...
      AppLogger.info(
          '[SIMPLIFIED_AUTH] 📝 Supabase sign-up successful for: ${supabaseUser.email}');
      ...
      AppLogger.info(
          '[SIMPLIFIED_AUTH] 📝 Sign-up complete for user: ${user.userId}');
      _userController.add(user);
  ...
  Future<User> googleSignInSimplified() async {
    if (!AuthFeatureFlags.useDirectSupabaseSignIn) {
      throw UnsupportedError(
          'Simplified Google sign-in is disabled by feature flag');
    }

    try {
      AppLogger.info('[SIMPLIFIED_AUTH] 🔍 Attempting native Google OAuth');
      AppLogger.info(
          '[SIMPLIFIED_AUTH] 🔍 Current session before OAuth: ${_supabase.auth.currentSession != null}');
      ...
      if (currentSession != null &&
        ...
        AppLogger.info(
            '[SIMPLIFIED_AUTH] ✅ User already authenticated with Google, fetching profile');
      ...
      if (idToken == null) {
        throw AuthException(
            'No ID token received from Google', 'GOOGLE_NO_TOKEN');
      }

      AppLogger.info(
          '[SIMPLIFIED_AUTH] 🔍 Got Google ID token, signing in with Supabase');
      ...
      if (response.user == null || response.session == null) {
        throw AuthException(
            'Google sign-in succeeded but no Supabase session created',
            'GOOGLE_NO_SESSION');
      }
      ...
      AppLogger.info(
          '[SIMPLIFIED_AUTH] ✅ Google OAuth successful for user: ${supabaseUser.email}');
  ...
  Future<User?> getCurrentUser({bool forceRefresh = false}) async {
    ...
    if (_inflightProfileRequest != null) {
      try {
        ...
        AppLogger.warning(
            '[SIMPLIFIED_AUTH] In-flight profile request failed: $e');
      ...
      switch (event) {
        case AuthChangeEvent.signedIn:
          if (session != null) {
            ...
            try {
              ...
              AppLogger.warning(
                  '[SIMPLIFIED_AUTH] Failed to fetch profile on sign-in: $e');
            }
          }
          break;
        ...
        case AuthChangeEvent.tokenRefreshed:
          if (session != null) {
            ...
            if (AuthFeatureFlags.enableDebugLogging) {
              AppLogger.info(
                  '[SIMPLIFIED_AUTH] 🔄 Token refreshed automatically');
            }
          }
          break;
  ...
  bool _isCacheValid() {
    ...
    return DateTime.now().difference(_lastProfileFetchAt!) <
