...
class InAppReviewService {
  ...
  Future<void> checkAndPromptAfterRuck({
    ...
    try {
      debugPrint(
          '[InAppReview] 🔍 Checking review prompt for ${distanceKm}km ruck');

      // Only prompt for qualifying rucks (1km or more)
      if (distanceKm < _qualifyingRuckDistanceKm) {
        debugPrint(
            '[InAppReview] ❌ Distance ${distanceKm}km too short, need >= ${_qualifyingRuckDistanceKm}km');
        return;
      }
      ...
      debugPrint(
          '[InAppReview] 📊 Total completed rucks: ${completedRucks + 1}');

      // Check if user has already left a review
      final hasLeftReview = prefs.getBool(_keyHasLeftReview) ?? false;
      if (hasLeftReview) {
        debugPrint(
            '[InAppReview] ✅ User has already left a review - skipping prompt');
        return;
      }
      ...
      debugPrint(
          '[InAppReview] 💯 Request count: $requestCount / $_maxReviewRequestsPerYear');
      if (requestCount >= _maxReviewRequestsPerYear) {
        debugPrint(
            '[InAppReview] ⚠️ Max review requests reached for this year');
        return;
      }
      ...
      if (lastRequestDate != null) {
        ...
        debugPrint(
            '[InAppReview] 📅 Days since last request: $daysSinceLastRequest / $_minDaysBetweenRequests');
      ...
      if (completedRucks + 1 == 1) {
        ...
        shouldPrompt = true;
        debugPrint(
            '[InAppReview] 🔥 5th ruck with no prior requests - will prompt!');
        ...
        debugPrint(
            '[InAppReview] 😴 Not the right time to prompt (ruck ${completedRucks + 1}, requests: $requestCount)');
      }
  ...
  Future<void> _updateReviewRequestTracking(SharedPreferences prefs) async {
    ...
    await prefs.setString(
        _keyLastReviewRequestDate, DateTime.now().toIso8601String());
  }
