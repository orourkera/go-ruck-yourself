...
class GooglePlacesService {
  ...
  Future<List<LocationSearchResult>> searchLocations(String query) async {
    ...
    if (!GooglePlacesConfig.isConfigured) {
      print(
          'GooglePlacesService: API key not configured. Please add your Google Places API key.');
      return [];
    }
    ...
    try {
      ...
      if (response.statusCode != 200) {
        print(
            'GooglePlacesService: API error ${response.statusCode}: ${response.body}');
        return [];
      }
      ...
      print(
          'GooglePlacesService: Returning ${searchResults.length} processed results');
  ...
  Future<List<LocationSearchResult>> searchAutocomplete(String query) async {
    ...
    try {
      ...
      if (response.statusCode != 200) {
        print(
            'GooglePlacesService: Autocomplete API error ${response.statusCode}');
        return [];
      }
      ...
      if (data['status'] != 'OK' && data['status'] != 'ZERO_RESULTS') {
        print(
            'GooglePlacesService: Autocomplete API status error: ${data['status']}');
        return [];
      }

      final List<dynamic> predictions = data['predictions'] ?? [];
      print(
          'GooglePlacesService: Found ${predictions.length} autocomplete predictions');
