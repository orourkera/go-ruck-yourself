...
class GPXExportService {
  ...
  Future<File> exportRouteToGPX(Route route, {String? customFileName}) async {
    ...
    return await _writeGPXFile(gpxContent, customFileName ?? '${route.name}.gpx');
  }
  
  /// 📅 **Export Planned Ruck to GPX File**
  /// 
  /// Creates a GPX file from a PlannedRuck with route data
  Future<File> exportPlannedRuckToGPX(PlannedRuck plannedRuck, {String? customFileName}) async {
    ...
    final fileName = customFileName ?? '${plannedRuck.route?.name ?? 'Planned Ruck'}_${plannedRuck.plannedDate.toLocal().toString().split(' ')[0]}.gpx';
  ...
  String _generateRouteGPX(Route route) {
    ...
    builder.element('gpx', nest: () {
      ...
      builder.attribute('xsi:schemaLocation', '$_gpxNamespace http://www.topografix.com/GPX/1/1/gpx.xsd');
      
      // Metadata
      builder.element('metadata', nest: () {
        builder.element('name', nest: route.name);
        builder.element('desc', nest: route.description ?? 'Exported from Ruck!');
        ...
        if (route.elevationPoints.isNotEmpty) {
          final coordinates = route.elevationPoints.where((p) => p.latitude != null && p.longitude != null)
        ...
        builder.element('trkseg', nest: () {
          for (int i = 0; i < route.elevationPoints.length; i++) {
            ...
            builder.element('trkpt', nest: () {
              ...
              builder.element('time', nest: DateTime.now().add(Duration(minutes: i)).toUtc().toIso8601String());
            });
          }
        });
      });
  ...
  String _generatePlannedRuckGPX(PlannedRuck plannedRuck) {
    ...
    builder.element('gpx', nest: () {
      ...
      builder.element('metadata', nest: () {
        builder.element('name', nest: plannedRuck.route?.name ?? 'Planned Ruck');
        builder.element('desc', nest: plannedRuck.notes ?? 'Planned ruck session');
        builder.element('time', nest: plannedRuck.plannedDate.toUtc().toIso8601String());
        
        // Custom extensions for ruck data
        builder.element('extensions', nest: () {
          builder.element('ruck_data', nest: () {
            builder.element('planned_date', nest: plannedRuck.plannedDate.toIso8601String());
            builder.element('duration', nest: plannedRuck.projectedDurationMinutes?.toString() ?? '0');
            builder.element('difficulty', nest: plannedRuck.route?.trailDifficulty ?? 'moderate');
            if (plannedRuck.targetWeight != null) {
              builder.element('target_weight', nest: plannedRuck.targetWeight.toString());
            }
          });
        });
      });
      ...
      builder.element('trk', nest: () {
        builder.element('name', nest: plannedRuck.route?.name ?? 'Planned Ruck');
        builder.element('desc', nest: plannedRuck.notes ?? route.description ?? 'Planned ruck route');
                ...
                builder.element('trkseg', nest: () {
                  for (final trkPt in child.findElements('trkpt')) {
                    builder.element('trkpt', nest: () {
                      ...
                      for (final ptChild in trkPt.children.whereType<XmlElement>()) {
                        builder.element(ptChild.name.local, nest: ptChild.innerText);
                      }
                    });
                  }
                });
  ...
  Future<String> exportToDocuments(Route route, {String? customName}) async {
    final directory = await getApplicationDocumentsDirectory();
    final fileName = customName ?? '${route.name}_${DateFormat('yyyy-MM-dd').format(DateTime.now())}.gpx';
    final file = File('${directory.path}/$fileName');
