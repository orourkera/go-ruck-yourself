...
class BatteryOptimizationService {
  static const String _ignoreBatteryOptimizationChecked =
      'ignore_battery_optimization_checked';

  /// Show custom dialog explaining battery optimization before requesting system permission
  static Future<bool> showBatteryOptimizationExplanation(
  ...
  static Future<bool> ensureBackgroundExecutionPermissions(
      {BuildContext? context}) async {
    if (!Platform.isAndroid) {
      AppLogger.info(
          '[BATTERY] Not Android - skipping battery optimization check');
      return true;
    }
    ...
    try {
      ...
      AppLogger.info(
          '[BATTERY] Battery optimization status: $batteryOptimizationStatus');
      AppLogger.info(
          '[BATTERY] Status details - isDenied: ${batteryOptimizationStatus.isDenied}, isGranted: ${batteryOptimizationStatus.isGranted}, isPermanentlyDenied: ${batteryOptimizationStatus.isPermanentlyDenied}');

      if (!batteryOptimizationStatus.isGranted) {
        AppLogger.info(
            '[BATTERY] Permission not granted (status: $batteryOptimizationStatus) - showing explanation dialog');
        // Show custom explanation dialog first if context is provided
        if (context != null) {
          final userAccepted =
              await showBatteryOptimizationExplanation(context);
          AppLogger.info('[BATTERY] User response to dialog: $userAccepted');
          if (!userAccepted) {
            AppLogger.info(
                '[BATTERY] User declined battery optimization exemption');
            return false;
          }
        } else {
          AppLogger.warning(
              '[BATTERY] No context provided - cannot show dialog');
        }

        AppLogger.info(
            '[BATTERY] Requesting battery optimization exemption...');
        final result = await Permission.ignoreBatteryOptimizations.request();
        AppLogger.info(
            '[BATTERY] Battery optimization request result: $result');
        return result.isGranted;
      } else {
        AppLogger.info(
            '[BATTERY] Permission already granted (status: $batteryOptimizationStatus) - no dialog needed');
      }

      final isGranted = batteryOptimizationStatus.isGranted;
      AppLogger.info(
          '[BATTERY] Final permission status - isGranted: $isGranted');
      return isGranted;
    } catch (e) {
      AppLogger.error(
          '[BATTERY] Error checking battery optimization permissions: $e');
      return false;
    }
  }
  ...
  static Future<Map<String, bool>> checkBackgroundLocationPermissions() async {
    ...
    try {
      ...
      // Overall result
      results['all_granted'] =
          results.values.every((granted) => granted == true);
  ...
  static Future<bool> requestAllBackgroundPermissions(
      {BuildContext? context}) async {
    if (!Platform.isAndroid) return true;
