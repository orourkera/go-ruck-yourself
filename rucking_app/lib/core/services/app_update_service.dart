...
class AppUpdateService {
  ...
  Future<UpdateInfo?> checkForUpdate() async {
    try {
      ...
      if (await _wasRecentlyChecked()) {
        AppLogger.debug(
            '[UPDATE_SERVICE] Update check was done recently, skipping');
        return null;
      }
      ...
      AppLogger.info(
          '[UPDATE_SERVICE] Latest available version: $latestVersion');
  ...
  Future<String?> _getLatestVersionFromBackend() async {
    try {
      ...
      final response = await _apiClient
          .get('/app/version-info', queryParams: {'platform': platform});
      return response['latest_version'] as String?;
    } catch (e) {
      AppLogger.warning(
          '[UPDATE_SERVICE] Failed to get version from backend: $e');
      return null;
    }
  }
  ...
  bool _hasActiveSession() {
    try {
      ...
      if (isActive) {
        ...
        AppLogger.warning(
            '[UPDATE_SERVICE] 🚫 Active ruck session detected ($statusText) - blocking updates');
        AppLogger.info(
            '[UPDATE_SERVICE] Session ID: ${sessionState.sessionId}, Duration: ${sessionState.elapsedSeconds}s, Distance: ${sessionState.distanceKm.toStringAsFixed(2)}km');
      }
  ...
  Future<bool> _isForceUpdateRequired(String currentVersion) async {
    try {
      ...
      if (forceUpdate && _hasActiveSession()) {
        AppLogger.critical(
            '[UPDATE_SERVICE] 🛡️ BLOCKING force update - active ruck session in progress');
      ...
      AppLogger.warning(
          '[UPDATE_SERVICE] Failed to check force update requirement: $e');
      return false;
    }
  }
  ...
  Future<bool> shouldShowUpdatePrompt(String latestVersion) async {
    try {
      ...
      if (_hasActiveSession()) {
        AppLogger.info(
            '[UPDATE_SERVICE] 🚫 Skipping update prompt - active ruck session in progress');
        return false;
      }
      ...
      if (dismissedVersion == latestVersion) {
        AppLogger.debug(
            '[UPDATE_SERVICE] User dismissed version $latestVersion');
        return false;
      }
      ...
      if (lastPromptedVersion == latestVersion) {
        AppLogger.debug(
            '[UPDATE_SERVICE] Already prompted for version $latestVersion');
        return false;
      }

      AppLogger.info(
          '[UPDATE_SERVICE] Should show update prompt for version $latestVersion');
      return true;
    } catch (e) {
      AppLogger.error(
          '[UPDATE_SERVICE] Error checking if should show prompt: $e');
      return false;
    }
  }
  ...
  Future<void> markPromptShown(String version) async {
    try {
      ...
      AppLogger.debug(
          '[UPDATE_SERVICE] Marked prompt shown for version $version');
    } catch (e) {
  ...
  Future<void> dismissUpdate(String version) async {
    try {
      ...
      AppLogger.info(
          '[UPDATE_SERVICE] User dismissed update for version $version');
    } catch (e) {
  ...
  Future<bool> _wasRecentlyChecked() async {
    try {
      ...
      final fourHoursAgo =
          now - (4 * 60 * 60 * 1000); // 4 hours in milliseconds
