...
class AppErrorHandler {
  ...
  static Future<void> handleError(
    ...
    if (error.toString().contains('kCLErrorDomain error 1') ||
      ...
      AppLogger.info(
          'Location permission denied - not reporting as error: $error');
      return;
    }
    ...
    if (error.toString().contains('403') ||
        ...
        error.toString().contains('forbidden')) {
      AppLogger.info(
          'Authorization error (403) - not reporting to Sentry: $error');
      return;
    }
    ...
    if (error.toString().contains('loss of GPU access') ||
      ...
      AppLogger.warning(
          'GPU memory issue detected - reducing image processing load: $error');
    ...
    if (_lastSentryError == null ||
        now.difference(_lastSentryError!) > _sentryRateLimitWindow) {
    ...
    if (_sentryErrorCount >= _maxSentryErrorsPerHour) {
      AppLogger.warning(
          'Sentry rate limit reached ($_maxSentryErrorsPerHour errors/hour) - skipping error report for: $operation');
      return;
    }
    ...
    try {
      ...
      if (severity == ErrorSeverity.error || severity == ErrorSeverity.fatal) {
        await Sentry.captureException(
          ...
          withScope: (scope) {
            ...
            scope.setTag(
                'operation_category', _getOperationCategory(operation));
      ...
      if (errorString.contains('rate') ||
        ...
        AppLogger.warning(
            'Sentry rate limit reached - error reporting temporarily disabled: $sentryError');
      } else if (errorString.contains('ApiException') ||
          errorString.contains('network')) {
        AppLogger.warning(
            'Sentry network error - error reporting failed: $sentryError');
      } else {
  ...
  static String _getOperationCategory(String operation) {
    ...
    if (operation.contains('avatar') || operation.contains('photo')) {
      ...
      return 'fitness';
    } else if (operation.contains('auth') ||
        operation.contains('login') ||
        operation.contains('logout')) {
      return 'authentication';
    } else if (operation.contains('social') ||
        operation.contains('like') ||
        operation.contains('comment')) {
      return 'social';
