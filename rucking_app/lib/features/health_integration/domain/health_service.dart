...
class HealthService {
  ...
  static const String _isHealthIntegrationEnabledKeyBase =
  ...
  Future<bool> requestAuthorization() async {
    try {
      ...
      AppLogger.info(
          'Calling health.requestAuthorization (types=${types.length})');
      final authorized =
      ...
      if (_isAuthorized) {
        ...
        AppLogger.warning(
            'Health authorization failed or step permission denied');
      }
  ...
  Future<bool> saveWorkout({
    ...
    AppLogger.info(
        'Saving workout with distance=${distanceKm * 1000}m, calories=$caloriesBurned');
    if (!_isAuthorized) {
      await requestAuthorization();
      if (!_isAuthorized) {
        return false;
    ...
    try {
      ...
      AppLogger.info(
          'Health data write results - Distance: $distanceSuccess, Calories: $caloriesSuccess, Workout: $workoutSuccess');
  ...
  Future<bool> isHealthIntegrationEnabled() async {
    if (_userId.isEmpty) {
      AppLogger.warning(
          'User ID not set when checking health integration status');
      return false;
    }

    final prefs = await SharedPreferences.getInstance();
    return prefs.getBool('${_userId}_$_isHealthIntegrationEnabledKeyBase') ??
        false;
  }
  ...
  Future<void> setHealthIntegrationEnabled(bool enabled) async {
    if (_userId.isEmpty) {
      AppLogger.warning(
          'User ID not set when setting health integration status');
      return;
    }

    final prefs = await SharedPreferences.getInstance();
    await prefs.setBool(
        '${_userId}_$_isHealthIntegrationEnabledKeyBase', enabled);
  }
  ...
  Future<bool> isLiveStepTrackingEnabled() async {
    final prefs = await SharedPreferences.getInstance();
    return prefs.getBool('live_step_tracking') ??
  ...
  Future<bool> hasAppleWatch() async {
    ...
    final prefs = await SharedPreferences.getInstance();
    return prefs.getBool('${_userId}_$_hasAppleWatchKeyBase') ??
  ...
  Future<double?> getHeartRate() async {
    try {
      ...
      AppLogger.info(
          'Most recent heart rate raw value: ${mostRecent.value} at ${mostRecent.dateFrom}');
  ...
  Future<int> getStepsBetween(DateTime start, DateTime end) async {
    ...
    try {
      ...
      if (!_isAuthorized) {
        AppLogger.info(
            '[STEPS DEBUG] Not authorized, requesting authorization...');
        final authorized = await requestAuthorization();
        AppLogger.info(
            '[STEPS DEBUG] Authorization request result: $authorized');
        if (!authorized) {
          AppLogger.warning(
              '[STEPS DEBUG] Authorization failed, returning 0 steps');
          return 0;
        }
      }
      ...
      if (authStatuses == null) {
        ...
        print(
            '[STEPS DEBUG] hasPermissions returned null; using iOS authorized status: $hasStepPermission');
      } else {
      ...
      if (hasStepPermission == false) {
        AppLogger.info(
            '[STEPS DEBUG] No step permission, requesting authorization...');
        final ok = await requestAuthorization();
      ...
      if (Platform.isIOS) {
        ...
        try {
          ...
          AppLogger.info(
              '[STEPS DEBUG] Time window: $start to $end (duration: ${end.difference(start).inMinutes} minutes)');
          final totalSteps = await _health.getTotalStepsInInterval(start, end);
          AppLogger.info(
              '[STEPS DEBUG] getTotalStepsInInterval returned: $totalSteps (type: ${totalSteps.runtimeType})');
          if (totalSteps != null && totalSteps > 0) {
            AppLogger.info(
                '[STEPS DEBUG] getTotalStepsInInterval success: $totalSteps');
            return totalSteps;
          } else {
            AppLogger.warning(
                '[STEPS DEBUG] getTotalStepsInInterval returned null or 0: $totalSteps');
          }
        } catch (e) {
        ...
        try {
          AppLogger.info(
              '[STEPS DEBUG] Method 2: Calling health.getHealthDataFromTypes...');
          ...
          AppLogger.info(
              '[STEPS DEBUG] Retrieved ${points.length} health data points');

          if (points.isEmpty) {
            AppLogger.warning(
                '[STEPS DEBUG] No health data points returned for time window');
            AppLogger.info(
                '[STEPS DEBUG] Checking broader time window to see if ANY step data exists...');
            ...
            try {
              ...
              AppLogger.info(
                  '[STEPS DEBUG] Last 24h test query returned ${testPoints.length} points');
              if (testPoints.isNotEmpty) {
                AppLogger.info(
                    '[STEPS DEBUG] Sample point: ${testPoints.first.dateFrom} to ${testPoints.first.dateTo}, value: ${testPoints.first.value}');
              }
            } catch (e) {
          ...
          for (int i = 0; i < points.length; i++) {
            ...
            final dynamic raw = p.value;
            AppLogger.debug(
                '[STEPS DEBUG] Point $i: ${p.dateFrom} to ${p.dateTo}, value: $raw (${raw.runtimeType})');

            if (raw is NumericHealthValue) {
              final value = (raw.numericValue ?? 0).toInt();
              total += value;
              AppLogger.debug(
                  '[STEPS DEBUG] NumericHealthValue: +$value, total: $total');
            } else if (raw is num) {
              ...
              if (parsed != null) {
                total += parsed;
                AppLogger.debug(
                    '[STEPS DEBUG] parsed: +$parsed, total: $total');
              } else {
          ...
          AppLogger.info(
              '[STEPS DEBUG] Final total from getHealthDataFromTypes: $total');
          return total;
        } catch (e) {
        ...
        try {
          AppLogger.info(
              '[STEPS DEBUG][Android] Calling health.getHealthDataFromTypes for STEPS...');
          final List<HealthDataPoint> points =
              await _health.getHealthDataFromTypes(
            startTime: start,
            endTime: end,
          ...
          AppLogger.info(
              '[STEPS DEBUG][Android] Retrieved ${points.length} points');
          if (points.isEmpty) {
            AppLogger.warning(
                '[STEPS DEBUG][Android] No step data points in range');
            return 0;
          }
          ...
          for (int i = 0; i < points.length; i++) {
            ...
            final dynamic raw = p.value;
            AppLogger.debug(
                '[STEPS DEBUG][Android] Point $i: ${p.dateFrom}→${p.dateTo} value=$raw (${raw.runtimeType})');
      ...
      AppLogger.warning(
          '[STEPS DEBUG] All step retrieval methods failed, returning 0');
      return 0;
    } catch (e) {
  ...
  int estimateStepsFromDistance(double distanceKm, {double? userHeightCm}) {
    ...
    final stepLengthM =
        (heightCm / 100) * 0.415; // Convert cm to m, then apply factor
    ...
    AppLogger.debug(
        '[STEPS FALLBACK] Distance: ${distanceKm}km, height: ${heightCm}cm, stepLength: ${stepLengthM}m, estimatedSteps: $estimatedSteps');
  ...
  Stream<int> startLiveSteps(DateTime start) {
    ...
    if (Platform.isIOS) {
      print(
          '[STEPS DEBUG] iOS detected - using CMPedometer for real-time steps');
    ...
    try {
      ...
      print(
          '[STEPS DEBUG] Watch session active: ${watchService.isSessionActive}');
      ...
      if (watchService.isSessionActive) {
        ...
        print(
            '[STEPS DEBUG] ✅ Setting up Apple Watch steps stream (session active)');
        ...
        timeoutTimer = Timer(const Duration(seconds: 15), () {
          if (!watchTimedOut) {
            print(
                '[STEPS DEBUG] ⚠️ TIMEOUT: No watch steps received in 15 seconds');
        ...
        AppLogger.warning(
            '[STEPS LIVE] ⚠️  Watch session not active, falling back to polling');
      }
    } catch (e) {
      AppLogger.warning(
          '[STEPS LIVE] WatchService not available, falling back to HealthKit polling: $e');
    }
    ...
    (() async {
      ...
      AppLogger.debug(
          '[STEPS LIVE] Immediate poll - session duration: ${sessionDuration.inSeconds}s');

      if (sessionDuration.inSeconds < 10) {
        // Too early - HealthKit likely hasn't recorded any intervals yet
        AppLogger.info(
            '[STEPS LIVE] Session too new (${sessionDuration.inSeconds}s), emitting 0 steps');
    ...
    _stepsTimer = Timer.periodic(const Duration(seconds: 10), (_) async {
      ...
      final sessionDuration = now.difference(sessionStart);
      print(
          '[STEPS DEBUG] ⏰ POLLING TIMER FIRED - session duration: ${sessionDuration.inSeconds}s');
      ...
      if (sessionDuration.inSeconds < 10) {
        print(
            '[STEPS DEBUG] Session too new (${sessionDuration.inSeconds}s), waiting for HealthKit data...');
        if (_stepsController != null && !_stepsController!.isClosed) {
          _stepsController!.add(0);
        }
        return;
      ...
      print(
          '[STEPS DEBUG] Querying HealthKit for steps between $sessionStart and $now');
      final total = await getStepsBetween(sessionStart, now);
      print('[STEPS DEBUG] 📊 HEALTHKIT RETURNED: $total steps');
      if (_stepsController != null && !_stepsController!.isClosed) {
        ...
        AppLogger.warning(
            '[STEPS LIVE] Steps controller is null/closed; skipping emit');
      }
    });
  ...
  void stopLiveSteps() {
    ...
    try {
      ...
      AppLogger.debug(
          '[STEPS LIVE] Pedometer subscription cancel (safe to ignore): $e');
    }
  ...
  Stream<int> _startCMPedometerStream(DateTime sessionStart) {
    ...
    const EventChannel pedometerChannel =
        EventChannel('com.getrucky.gfy/pedometerStream');

    // Store the subscription so we can cancel it properly
    _pedometerSubscription =
        pedometerChannel.receiveBroadcastStream().map<int>((dynamic event) {
      ...
      if (event is Map) {
        ...
        print(
            '[STEPS DEBUG] 📊 CMPedometer - Steps: $steps, Distance: ${distance?.toStringAsFixed(1)}m');
      ...
      print(
          '[STEPS DEBUG] Unexpected pedometer data format: ${event.runtimeType}');
      return 0;
    }).listen((int steps) {
  ...
  void _startHealthKitPollingWithSink(
      DateTime sessionStart, EventSink<int> sink) {
    print('[STEPS DEBUG] 🔄 STARTING HEALTHKIT POLLING FALLBACK');
    ...
    _stepsTimer = Timer.periodic(const Duration(seconds: 5), (timer) async {
      ...
      try {
        ...
        if (sessionDuration.inSeconds < 5) {
          print(
              '[STEPS DEBUG] [HEALTHKIT_FALLBACK] Session too new (${sessionDuration.inSeconds}s), waiting for HealthKit data...');
          sink.add(0);
          return;
        }
        ...
        final actualStart =
            sessionStart.isAfter(fallbackStart) ? sessionStart : fallbackStart;

        print('[STEPS DEBUG] Querying HealthKit from $actualStart to $now');
        final total = await getStepsBetween(actualStart, now);
        print(
            '[STEPS DEBUG] [HEALTHKIT_FALLBACK] HealthKit returned: $total steps');

        // Try to add data - if sink is closed, this will throw
        try {
          sink.add(total);
          print(
              '[STEPS DEBUG] [HEALTHKIT_FALLBACK] Successfully sent $total steps to stream');
        } catch (e) {
          print(
              '[STEPS DEBUG] [HEALTHKIT_FALLBACK] Sink closed, stopping timer: $e');
          timer.cancel();
        }
      } catch (e) {
        print(
            '[STEPS DEBUG] [HEALTHKIT_FALLBACK] Error during step polling: $e');
