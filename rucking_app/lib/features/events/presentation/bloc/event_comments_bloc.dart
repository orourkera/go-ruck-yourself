...
class EventCommentsBloc extends Bloc<EventCommentsEvent, EventCommentsState> {
  ...
  Future<void> _onLoadEventComments(
      LoadEventComments event, Emitter<EventCommentsState> emit) async {
    try {
  ...
  Future<void> _onRefreshEventComments(
      RefreshEventComments event, Emitter<EventCommentsState> emit) async {
    add(LoadEventComments(event.eventId));
  }

  Future<void> _onAddEventComment(
      AddEventComment event, Emitter<EventCommentsState> emit) async {
    try {
  ...
  Future<void> _onUpdateEventComment(
      UpdateEventComment event, Emitter<EventCommentsState> emit) async {
    try {
      emit(EventCommentActionLoading(
        eventId: event.eventId,
  ...
  Future<void> _onDeleteEventComment(
      DeleteEventComment event, Emitter<EventCommentsState> emit) async {
    try {
      emit(EventCommentActionLoading(
        eventId: event.eventId,
