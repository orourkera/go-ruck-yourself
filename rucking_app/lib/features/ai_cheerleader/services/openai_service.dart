...
class OpenAIService {
    ...
    try {
      ...
      if (message != null && message.isNotEmpty) {
        ...
        final emojiRegex = RegExp(
            r"[\u{1F300}-\u{1FAFF}\u{1F600}-\u{1F64F}\u{2600}-\u{26FF}\u{2700}-\u{27BF}]",
            unicode: true);
  ...
  Future<String?> generateStravaTitle({
    ...
    try {
      ...
      final timeOfDay =
          startTime != null ? DateFormat('EEEE').format(startTime) : '';
  ...
  Future<String?> generateSessionSummary({
    ...
    try {
      ...
      AppLogger.error(
          '[OPENAI][SUMMARY] Network error - no internet connection');
      return null;
    } catch (e, st) {
  ...
  Future<String?> generateSessionSummaryWithCoachingContext({
    ...
    try {
      ...
      final prompt =
          _buildSessionSummaryPrompt(context, coachingPlan: coachingPlan);
  ...
  String _buildSessionSummaryPrompt(Map<String, dynamic> context,
    ...
    try {
      ...
      systemPrompt = remoteConfig.getString('ai_session_summary_system_prompt',
          fallback:
              '''You are an expert fitness analyst generating a concise post-ruck session summary.
...
Focus on factual metrics, notable achievements, and one meaningful insight. Keep it clear and encouraging without hype.''';
    ...
    if (preferMetric) {
      if (distanceKm != null)
        sb.writeln(
            '  "distance_km": ${distanceKm.toStringAsFixed(distanceKm >= 10 ? 0 : 2)},');
    } else {
      if (distanceMi != null)
        sb.writeln(
            '  "distance_miles": ${distanceMi.toStringAsFixed(distanceMi >= 10 ? 0 : 2)},');
    }
    ...
    if (preferMetric) {
      if (elevGain != null)
        sb.writeln('  "elev_gain_m": ${elevGain.toStringAsFixed(0)},');
      if (elevLoss != null)
        sb.writeln('  "elev_loss_m": ${elevLoss.toStringAsFixed(0)},');
    } else {
      if (elevGain != null) {
        final elevGainFt = elevGain *
            3.28084; // MeasurementUtils doesn't have direct elevation conversion method
        sb.writeln('  "elev_gain_ft": ${elevGainFt.toStringAsFixed(0)},');
      }
      if (elevLoss != null) {
        final elevLossFt = elevLoss *
            3.28084; // MeasurementUtils doesn't have direct elevation conversion method
    ...
    if (context['heart_rate_zones'] is Map)
...
When the plan includes scientific sources, occasionally cite them to build trust (e.g., "Your plan follows Daniels' Running Formula principles for aerobic development").'''
    ...
    final guidelines = [
      ...
      coachingPlan != null
          ? '- PRIORITY: Include coaching plan context - reference plan progress and adherence.'
          : '',
  ...
  String _buildPrompt(
    ...
    final personalityPrompt =
        _getPersonalityPrompt(personality, explicitContent);
    final baseContext =
        _buildBaseContext(trigger, session, user, environment, history);
...
Respond with your motivational message:''';
  ...
  String _buildBaseContext(
    ...
    String contextText =
        "Rucking session: ${elapsedTime} elapsed, ${distance} covered.";

    switch (triggerType) {
      case 'milestone':
        final milestone = triggerData['milestone'];
        final distanceObj = session is Map ? session['distance'] : null;
        final unit = distanceObj is Map && distanceObj['unit'] != null
            ? distanceObj['unit']
            : 'km';
        if (milestone is num) {
      ...
      case 'heartRateSpike':
        ...
        contextText +=
            " Heart rate spike: ${hr}bpm (baseline ~${baseline}bpm). Encourage breathing rhythm and steady form.";
        break;
      case 'timeCheckIn':
    ...
    final preferMetric = (user is Map && user['preferMetric'] is bool)
    ...
    final insight = _deriveOneHistoryInsight(
        session: session, history: history, preferMetric: preferMetric);
    if (insight != null && insight.isNotEmpty) {
      contextText += " $insight";
    }

    // Add rich history context for AI to reference
    final recentRucks = (history is Map && history['recent_rucks'] is List)
        ? history['recent_rucks'] as List
        : <dynamic>[];
    final achievements = (history is Map && history['achievements'] is List)
        ? history['achievements'] as List
        : <dynamic>[];
    ...
    if (coachingPlan != null && coachingPlan.isNotEmpty) {
      ...
      if (nextSession is Map) {
        ...
        if (sessionDistance != null) {
          final distance =
              preferMetric ? sessionDistance : (sessionDistance * 0.621371);
  ...
  void _logSimpleResponse({
    ...
    AppLogger.info(
        '[AI_LOG] _logSimpleResponse called - logger is null: ${_logger == null}');
    if (_logger == null) {
    ...
    try {
      ...
      AppLogger.error(
          '[AI_LOG_DEBUG] Got sessionId from ActiveSessionBloc: $sessionId');

      if (sessionId == null || sessionId.isEmpty) {
        // This is expected for homepage insights or any context outside an active ruck.
        AppLogger.warning(
            '[AI_LOG_DEBUG] No active session ID; skipping AI log.');
        AppLogger.warning(
            '[AI_LOG_DEBUG] ActiveSessionBloc state: ${activeState.runtimeType}');
        return;
      }

      AppLogger.info(
          '[AI_LOG] About to log response - sessionId: $sessionId, personality: $personality');
      _logger!.logResponse(
        sessionId: sessionId,
        personality: personality,
  ...
  String? _deriveOneHistoryInsight({
    ...
    final currentWeightKg = _readNum(
            session is Map ? Map<String, dynamic>.from(session) : null,
            ['gear', 'ruckWeightKg']) ??
        _readNum(session is Map ? Map<String, dynamic>.from(session) : null,
            ['weightKg']);
    final lastWeightKg = _readNum(lastMap, ['ruck_weight_kg']) ??
        _readNum(lastMap, ['weight_kg']);
    ...
    final currentPace = _readNum(
        session is Map ? Map<String, dynamic>.from(session) : null,
        ['performance', 'pace']); // seconds per unit
    final lastPace = _readNum(lastMap, ['avg_pace_seconds_per_km']) ??
        _readNum(lastMap, ['avg_pace_seconds_per_mile']);
    if (currentPace != null && lastPace != null) {
      final diffSec = currentPace - lastPace;
      if (diffSec.abs() >= 5) {
        // meaningful difference >= 5s
        final unit = preferMetric ? 'km' : 'mi';
        return diffSec < 0
    ...
    final sessionSplits = (session is Map && session['splits'] is List)
        ? session['splits'] as List
        : <dynamic>[];
    final lastSplits =
        (lastMap['splits'] is List) ? lastMap['splits'] as List : <dynamic>[];
    if (sessionSplits.isNotEmpty && lastSplits.isNotEmpty) {
      ...
      if (curLast is Map && prevLast is Map) {
        final curDur = _readNum(
            Map<String, dynamic>.from(curLast), ['splitDurationSeconds']);
        final prevDur = _readNum(
            Map<String, dynamic>.from(prevLast), ['splitDurationSeconds']);
  ...
  List<String> _recentAICheerleaderLines(Map<dynamic, dynamic> history,
      {int max = 2}) {
    AppLogger.error(
        '[ANTI_REPEAT_DEBUG] _recentAICheerleaderLines called with max=$max');
    AppLogger.error(
        '[ANTI_REPEAT_DEBUG] History keys: ${history.keys.toList()}');
    AppLogger.error(
        '[ANTI_REPEAT_DEBUG] ai_cheerleader_history type: ${history['ai_cheerleader_history'].runtimeType}');

    final items = (history['ai_cheerleader_history'] as List?) ?? const [];
    AppLogger.error(
        '[ANTI_REPEAT_DEBUG] Found ${items.length} items in ai_cheerleader_history');

    final lines = <String>[];
    for (int i = 0; i < items.length; i++) {
      final it = items[i];
      AppLogger.error(
          '[ANTI_REPEAT_DEBUG] Item $i: ${it.runtimeType} - ${it is Map ? (it as Map).keys.toList() : 'not a map'}');
      if (it is Map && it['openai_response'] is String) {
        var t = (it['openai_response'] as String).trim();
        AppLogger.error(
            '[ANTI_REPEAT_DEBUG] Found AI response: "${t.substring(0, math.min(50, t.length))}..."');
    ...
    AppLogger.error(
        '[ANTI_REPEAT_DEBUG] Extracted ${lines.length} previous responses from history');

    // If not enough history, supplement with local cache of recent lines
    if (lines.length < max && _localRecentLines.isNotEmpty) {
      AppLogger.error(
          '[ANTI_REPEAT_DEBUG] Supplementing with ${_localRecentLines.length} local cached lines');
