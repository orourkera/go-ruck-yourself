...
class OpenAIResponsesService {
  ...
  Future<void> stream({
    ...
    if (instructions != null && input != null) {
      ...
      throw ArgumentError('Must provide either input, or instructions+input, or messages');
    }
    ...
    if (!isO3 && !isO1 && temperature != null) body['temperature'] = temperature;
    ...
    AppLogger.debug('[OPENAI_SSE] Request body: ${jsonEncode(body).substring(0, jsonEncode(body).length.clamp(0, 500))}...');
        ...
        try {
          ...
          if (type == 'response.output_text.delta') {
            ...
            if (delta.isNotEmpty) {
              ...
              AppLogger.debug('[OPENAI_SSE] Got delta: ${delta.substring(0, delta.length.clamp(0, 50))}...');
            }
            ...
            if (item != null) {
              ...
              AppLogger.debug('[OPENAI_SSE] Found item object of type: $itemType');
              ...
              if (itemType == 'message') {
                ...
                if (itemText.isNotEmpty) {
                  full.clear();
                  full.write(itemText);
                  AppLogger.info('[OPENAI_SSE] Extracted ${itemText.length} chars from message item');
                } else {
                  AppLogger.debug('[OPENAI_SSE] Message item keys: ${item.keys.toList()}');
                }
              } else if (itemType == 'reasoning') {
                AppLogger.debug('[OPENAI_SSE] Skipping reasoning item (type: $itemType)');
              } else {
                AppLogger.debug('[OPENAI_SSE] Unknown item type: $itemType, keys: ${item.keys.toList()}');
              }
            }
            ...
            if (response != null && full.isEmpty) {
              AppLogger.debug('[OPENAI_SSE] Found response object, extracting text...');
              String finalText = _extractTextFromResponsesAPI(response);
              if (finalText.isNotEmpty) {
                full.clear();
                full.write(finalText);
                AppLogger.info('[OPENAI_SSE] Extracted ${finalText.length} chars from response object');
              }
            }
            ...
            if (full.isEmpty) {
              ...
              if (eventText.isNotEmpty) {
                ...
                AppLogger.info('[OPENAI_SSE] Extracted ${eventText.length} chars from event object');
              }
            }
            ...
            if (full.isEmpty) {
              final response = map['response'] as Map<String, dynamic>?;
              if (response != null) {
                AppLogger.debug('[OPENAI_SSE] Trying to extract final text from completed response...');
                
                // The response has an 'output' field containing the message items
                final output = response['output'] as List<dynamic>?;
                if (output != null) {
                  AppLogger.debug('[OPENAI_SSE] Found output array with ${output.length} items');
                  ...
                  for (int i = 0; i < output.length; i++) {
                    ...
                    if (item != null) {
                      final itemType = item['type']?.toString() ?? '';
                      AppLogger.debug('[OPENAI_SSE] Output item $i: type=$itemType, keys=${item.keys.toList()}');
                      
                      if (itemType == 'message') {
                        // Extract text from message content based on official response structure
                        final content = item['content'] as List<dynamic>?;
                        if (content != null) {
                          AppLogger.debug('[OPENAI_SSE] Message content has ${content.length} items');
                          for (int j = 0; j < content.length; j++) {
                            final contentItem = content[j];
                            if (contentItem is Map<String, dynamic>) {
                              final contentType = contentItem['type']?.toString() ?? '';
                              AppLogger.debug('[OPENAI_SSE] Content item $j: type=$contentType');
                              ...
                              if (contentType == 'output_text') {
                                ...
                                if (text is String && text.isNotEmpty) {
                                  full.clear();
                                  full.write(text);
                                  AppLogger.info('[OPENAI_SSE] Extracted ${text.length} chars from output_text content');
                                  break;
                                }
                              }
                            }
                ...
                if (full.isEmpty) {
                  ...
                  if (textField is String && textField.isNotEmpty) {
                    ...
                    AppLogger.info('[OPENAI_SSE] Extracted ${textField.length} chars from top-level text field');
                    ...
                    if (textContent != null && textContent.isNotEmpty) {
                      ...
                      AppLogger.info('[OPENAI_SSE] Extracted ${textContent.length} chars from text.content field');
                    } else {
                      AppLogger.debug('[OPENAI_SSE] Text field keys: ${textField.keys.toList()}');
                    }
                  } else {
                    AppLogger.error('[OPENAI_SSE] Text field type: ${textField.runtimeType}, value: $textField');
                  }
                }
                
                if (full.isEmpty) {
                  AppLogger.error('[OPENAI_SSE] Still no text found after checking all fields');
                }
              } else {
                AppLogger.error('[OPENAI_SSE] No response object in completed event');
              }
            }
          } else {
          ...
          AppLogger.warning('[OPENAI_SSE] Failed to parse SSE line: $e. Line: $dataStr');
        }
      });
  ...
  String _extractTextFromResponsesAPI(Map<String, dynamic> data) {
    ...
    if (result.isNotEmpty) {
      AppLogger.debug('[OPENAI_SSE] Extracted text: ${result.substring(0, result.length.clamp(0, 100))}...');
    }
