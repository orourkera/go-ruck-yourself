...
class SplitTrackingService {
  ...
  Future<void> checkForMilestone({
    ...
    try {
      ...
      final double splitDistanceKm =
          preferMetric ? 1.0 : 1.609; // 1km or 1mi (in km)

      AppLogger.debug(
          '[SPLITS] Split distance in km: $splitDistanceKm (${preferMetric ? "1km" : "1mi"})');

      // Calculate current milestone based on total distance
      final int currentMilestoneIndex =
          (currentDistanceKm / splitDistanceKm).floor();
      final double nextMilestoneDistanceKm =
          (currentMilestoneIndex + 1) * splitDistanceKm;
      ...
      if (currentMilestoneIndex > 0 &&
        ...
        final double splitElevationGain =
            _calculateSplitElevationGain(currentElevationGain);
        ...
        final splitInfo = {
          ...
          'split_duration_seconds':
              splitDuration.inSeconds, // Convert Duration to seconds
          'total_distance': preferMetric
              ? currentDistanceKm
              : currentDistanceKm / 1.609, // Convert to mi if needed
          'total_duration_seconds':
              elapsedSeconds, // Use seconds instead of Duration
          'calories_burned': splitCalories,
          'elevation_gain_m': splitElevationGain,
          'timestamp':
              splitEndTime.toIso8601String(), // Convert DateTime to string
        };
        ...
        await _watchService.sendSplitNotification(
          splitDistance:
              1.0, // Always show as 1.0 (represents 1km or 1mi based on isMetric flag)
          splitDuration: splitDuration,
          totalDistance: preferMetric
              ? currentDistanceKm
              : currentDistanceKm / 1.609, // Convert to mi if needed
        ...
        AppLogger.sessionCompletion('Split notification sent to watch',
            context: {
              'split_number': currentMilestoneIndex,
            });
      }
    } catch (e) {
      AppLogger.sessionCompletion('Error checking for distance milestone',
          context: {
            'error': e.toString(),
            'current_distance_km': currentDistanceKm,
          });
  ...
  double _calculateSplitCalories({
    ...
    try {
      ...
      } else if (paceMinPerKm <= 12.0) {
        // Moderate pace (8-12 min/km)
      ...
      AppLogger.debug(
          '[SPLITS] Calculated split calories: $calories (pace: ${paceMinPerKm.toStringAsFixed(1)} min/km, MET: $metValue)');
  ...
  double _calculateSplitElevationGain(double? currentElevationGain) {
    ...
    if (_lastSplitElevationM == 0.0) {
      ...
      AppLogger.debug(
          '[SPLITS] First split elevation gain: ${currentElevationGain.toStringAsFixed(1)}m');
      return currentElevationGain.clamp(
          0.0, double.infinity); // Only positive elevation gains
    }

    // Calculate elevation gain for this split
    final double splitElevationGain =
        currentElevationGain - _lastSplitElevationM;

    // Update the last split elevation for next calculation
    _lastSplitElevationM = currentElevationGain;

    AppLogger.debug(
        '[SPLITS] Split elevation gain: ${splitElevationGain.toStringAsFixed(1)}m (current: ${currentElevationGain.toStringAsFixed(1)}m, previous: ${(_lastSplitElevationM - splitElevationGain).toStringAsFixed(1)}m)');

    return splitElevationGain.clamp(
        0.0, double.infinity); // Only positive elevation gains
  }
}
