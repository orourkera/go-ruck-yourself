...
class HeartRateService {
  ...
  StreamController<List<HeartRateSample>> _bufferController =
      StreamController<List<HeartRateSample>>.broadcast();
  ...
  Stream<List<HeartRateSample>> get heartRateBufferStream =>
  ...
  Future<void> startHeartRateMonitoring() async {
    AppLogger.error(
        'HeartRateService: [HR_DEBUG] 🚀 STARTING HEART RATE MONITORING...');

    // Reset monitoring state to ensure fresh start for new session
    if (_isMonitoringStarted) {
      AppLogger.error(
          'HeartRateService: [HR_DEBUG] 🔄 Heart rate monitoring already active, resetting for new session');
    ...
    if (!_healthService.isAuthorized) {
      ...
      if (!granted) {
        AppLogger.warning(
            'HeartRateService: Health authorization denied – proceeding with Watch-only heart-rate stream');
    ...
    if (_healthService.isAuthorized) {
      ...
      AppLogger.info(
          'HeartRateService: Skipping HealthKit heart rate subscription (not authorized)');
    }
    ...
    AppLogger.error(
        'HeartRateService: [HR_DEBUG] ✅ HEART RATE MONITORING SUCCESSFULLY STARTED');

    // Log final state for debugging
    AppLogger.error(
        'HeartRateService: [HR_DEBUG] 📊 Final state - Watch subscription: ${_watchHeartRateSubscription != null}, HealthKit authorized: ${_healthService.isAuthorized}, Monitoring active: $_isMonitoringStarted');
    ...
    Future.delayed(const Duration(seconds: 4), () async {
      ...
      if (current == null || current <= 0) {
        AppLogger.error(
            'HeartRateService: [HR_DEBUG] ⏰ No HR yet after 4s – sending another ensureHeartRateStreaming');
        try {
          await _watchService.ensureHeartRateStreaming();
        } catch (_) {}
      }
  ...
  void _setupWatchHeartRateSubscription() {
    AppLogger.error(
        'HeartRateService: [HR_DEBUG] 🎧 SETTING UP WATCH HEART RATE SUBSCRIPTION');
    ...
    AppLogger.error(
        'HeartRateService: [HR_DEBUG] 📱 Current WatchService HR at subscription: $currentHR BPM');
    AppLogger.error(
        'HeartRateService: [HR_DEBUG] 📱 WatchService onHeartRateUpdate stream available: ${_watchService.onHeartRateUpdate != null}');

    _watchHeartRateSubscription = _watchService.onHeartRateUpdate.listen(
      (heartRate) {
        AppLogger.error(
            'HeartRateService: [HR_DEBUG] 🎯 RECEIVED RAW HEART RATE from WatchService: $heartRate BPM');
        // Validate heart rate before processing
        if (heartRate <= 0) {
          AppLogger.error(
              'HeartRateService: [HR_DEBUG] ❌ INVALID heart rate received from watch: $heartRate BPM');
          return;
        }
        ...
        if (_healthHeartRateSubscription != null) {
          ...
          _healthHeartRateSubscription = null;
          AppLogger.error(
              'HeartRateService: [HR_DEBUG] 🛑 DISABLED HealthKit subscription - using watch readings only');
        }
        ...
        AppLogger.error(
            'HeartRateService: [HR_DEBUG] ✅ PROCESSING valid heart rate sample: ${sample.bpm} BPM');
        _processHeartRateSample(sample, 'Watch');
      },
      onError: (error) {
        AppLogger.error(
            'HeartRateService: [HR_DEBUG] ❌ ERROR in watch heart rate stream: $error');
        // Auto-reconnect after error
        _tryReconnectWatchService();
      },
      onDone: () {
        AppLogger.error(
            'HeartRateService: [HR_DEBUG] ⚠️ WATCH HEART RATE STREAM CLOSED unexpectedly');
    ...
    if (currentHR != null && currentHR > 0) {
      AppLogger.error(
          'HeartRateService: [HR_DEBUG] ⚡ PROCESSING existing heart rate immediately: $currentHR BPM');
      final sample = HeartRateSample(
        timestamp: DateTime.now(),
        bpm: currentHR.toInt(),
      );
      ...
      AppLogger.error(
          'HeartRateService: [HR_DEBUG] 📭 No existing heart rate from WatchService (currentHR: $currentHR)');
    }

    AppLogger.error(
        'HeartRateService: [HR_DEBUG] ✅ WATCH HEART RATE SUBSCRIPTION SUCCESSFULLY SET UP');

    // Add a test to verify the stream is working
    Future.delayed(const Duration(seconds: 5), () {
      AppLogger.error(
          'HeartRateService: [HR_DEBUG] 🔍 STREAM CHECK: Subscription active: ${_watchHeartRateSubscription != null}, WatchService current HR: ${_watchService.getCurrentHeartRate()}');
    });

    // Nudge the watch to begin streaming HR immediately, and retry for ~30s until a sample arrives
    try {
      _watchService.ensureHeartRateStreaming();
    } catch (_) {}
    int attempts = 0;
    _hrKickTimer?.cancel();
    _hrKickTimer = Timer.periodic(const Duration(seconds: 5), (t) async {
      if (_lastHeartRateTime != null &&
        ...
        AppLogger.error(
            'HeartRateService: [HR_DEBUG] ✅ HR streaming confirmed; stopping kick timer');
        t.cancel();
        _hrKickTimer = null;
        return;
      }
      ...
      if (attempts > 6) {
        AppLogger.error(
            'HeartRateService: [HR_DEBUG] ⛔ HR kick attempts exceeded; giving up');
        t.cancel();
        _hrKickTimer = null;
        return;
      }
      AppLogger.error(
          'HeartRateService: [HR_DEBUG] ⏰ No HR yet – sending ensureHeartRateStreaming (attempt $attempts)');
      try {
        await _watchService.ensureHeartRateStreaming();
      } catch (_) {}
    });
  ...
  void _setupHealthKitHeartRateSubscription() {
    AppLogger.info(
        'HeartRateService: Setting up HealthKit heart rate subscription');
    ...
    _healthHeartRateSubscription = _healthService.heartRateStream.listen(
      ...
      onError: (error) {
        AppLogger.error(
            'HeartRateService: Error in HealthKit heart rate stream: $error');
      },
  ...
  void _tryReconnectWatchService() {
    ...
    AppLogger.info(
        'HeartRateService: Attempting to reconnect watch heart rate subscription');
    ...
    Future.delayed(const Duration(seconds: 2), () {
      ...
      AppLogger.info(
          'HeartRateService: Watch heart rate subscription reconnected');
    });
  }
  ...
  void _startConnectionWatchdog() {
    ...
    _watchdogTimer = Timer.periodic(const Duration(seconds: 10), (_) {
      ...
      if (_lastHeartRateTime != null &&
          ...
          !_isReconnecting) {
        AppLogger.warning(
            'HeartRateService: No heart rate updates received in the last 20 seconds');
        _tryReconnectWatchService();
      }
    });
  }
  ...
  Future<void> _fetchInitialHeartRate() async {
    try {
      ...
      if (initialHr != null && initialHr > 0) {
        AppLogger.info(
            'HeartRateService: Initial heart rate fetched: $initialHr BPM');
        final sample = HeartRateSample(
          timestamp: DateTime.now(),
          bpm: initialHr.round(),
        );
      ...
      AppLogger.error(
          'HeartRateService: Error fetching initial heart rate: $e');
    }
  }
  ...
  void _processHeartRateSample(HeartRateSample sample, String source) {
    ...
    if (sample.bpm <= 0) {
      AppLogger.error(
          'HeartRateService: [HR_DEBUG] ❌ IGNORING invalid heart rate value: ${sample.bpm} from $source');
      return;
    }
    ...
    AppLogger.error(
        'HeartRateService: [HR_DEBUG] 💓 RECEIVED heart rate update from $source: ${sample.bpm} BPM');
    ...
    if (shouldSave) {
      ...
      AppLogger.info(
          'HeartRateService: Saved heart rate sample to buffer (downsampled): ${sample.bpm} BPM');
    } else {
      AppLogger.debug(
          'HeartRateService: Skipped saving heart rate sample (downsampling): ${sample.bpm} BPM');
    }
    ...
    if (!_heartRateController.isClosed) {
      ...
      AppLogger.error(
          'HeartRateService: [HR_DEBUG] 📢 SUCCESSFULLY BROADCAST heart rate: ${sample.bpm} BPM to ${_heartRateController.hasListener ? 'ACTIVE LISTENERS' : 'NO LISTENERS ⚠️'}');
    } else {
      AppLogger.error(
          'HeartRateService: [HR_DEBUG] ❌ CANNOT broadcast heart rate sample - controller is CLOSED!');
      ...
      if (!_heartRateController.isClosed) {
        _heartRateController.add(sample);
        AppLogger.error(
            'HeartRateService: [HR_DEBUG] 🔄 BROADCAST heart rate after controller recreation: ${sample.bpm} BPM');
      }
    }
  ...
  Future<void> flushHeartRateBuffer() async {
    ...
    try {
      ...
      AppLogger.error(
          'HeartRateService: Failed to flush heart rate buffer: $e');
    }
  }
