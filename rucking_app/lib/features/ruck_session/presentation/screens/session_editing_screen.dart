...
class _SessionEditingScreenState extends State<SessionEditingScreen>
  ...
  void _initializeSessionData() {
    try {
      ...
      AppLogger.info(
          '[SESSION_EDITING] Initialized with ${_originalLocationPoints.length} location points');
    } catch (e) {
      AppLogger.error('[SESSION_EDITING] Error initializing session data',
          exception: e);
    }
  }
  ...
  void _updateTimelinePosition(double position) {
    ...
    setState(() {
      ...
      print(
          ' [SESSION_EDITING] Calling _calculatePreviewSession() with timeline position: $_timelinePosition');
        ...
        try {
          ...
          if (point.containsKey('latitude') && point.containsKey('longitude')) {
            ...
            if (timestampRaw is String) {
              ts = DateTime.tryParse(timestampRaw) ??
                  widget.originalSession.startTime;
          ...
          AppLogger.warning(
              '[SESSION_EDITING] Error parsing location point: $e');
          // Skip invalid points
        }
  ...
  void _calculatePreviewSession() {
    AppLogger.debug(
        '[SESSION_EDITING] _calculatePreviewSession called with timeline position: $_timelinePosition');
    try {
      if (_originalLocationPoints.isEmpty) {
        AppLogger.debug(
            '[SESSION_EDITING] No original location points, using original session');
        _previewSession = widget.originalSession;
        return;
      }
      ...
      final cutoffDuration = Duration(
        milliseconds:
            (totalDuration.inMilliseconds * _timelinePosition).round(),
      );
      ...
      final hasValidTimestamps = _originalLocationPoints.length >= 2 &&
          !_originalLocationPoints.every((point) => point.timestamp
              .isAtSameMomentAs(_originalLocationPoints.first.timestamp));
      ...
      if (hasValidTimestamps) {
        ...
        if (_timelinePosition >= 1.0) {
          ...
          print(
              '🔍 [SESSION_EDITING] Timeline position >= 1.0, keeping ALL ${pointsToKeep} points');
        } else {
          // Use ceiling to ensure we don't under-filter
          pointsToKeep =
              (_originalLocationPoints.length * _timelinePosition).ceil();
          print(
              '🔍 [SESSION_EDITING] Timeline position ${_timelinePosition}, calculated ${pointsToKeep} points to keep');
        }
        filteredLocationPoints =
            _originalLocationPoints.take(pointsToKeep).toList();
        print(
            '🔍 [SESSION_EDITING] Using position-based filtering (invalid timestamps)');
        print(
            '🔍 [SESSION_EDITING] Keeping ${pointsToKeep} out of ${_originalLocationPoints.length} points');
        print(
            '🔍 [SESSION_EDITING] Actual filtered points: ${filteredLocationPoints.length}');
      }

      // Debug timestamp filtering
      print(' [SESSION_EDITING] === TIMESTAMP DEBUG ===');
      print(
          ' [SESSION_EDITING] Session start: ${widget.originalSession.startTime}');
      print(
          ' [SESSION_EDITING] Session end: ${widget.originalSession.endTime}');
      ...
      if (_originalLocationPoints.isNotEmpty) {
        print(
            ' [SESSION_EDITING] First location point: ${_originalLocationPoints.first.timestamp}');
        print(
            ' [SESSION_EDITING] Last location point: ${_originalLocationPoints.last.timestamp}');
        print(
            ' [SESSION_EDITING] Cutoff is before last point: ${cutoffTime.isBefore(_originalLocationPoints.last.timestamp)}');
        print(
            ' [SESSION_EDITING] Time diff session vs locations: ${_originalLocationPoints.first.timestamp.difference(widget.originalSession.startTime).inSeconds}s');
      }

      AppLogger.debug(
          '[SESSION_EDITING] Original location points: ${_originalLocationPoints.length}, Filtered: ${filteredLocationPoints.length}, Timeline pos: $_timelinePosition, Cutoff time: $cutoffTime');
      AppLogger.debug(
          '[SESSION_EDITING] Session start: ${widget.originalSession.startTime}, Session end: ${widget.originalSession.endTime}, Total duration: ${totalDuration.inSeconds}s');
      AppLogger.debug(
          '[SESSION_EDITING] First point timestamp: ${_originalLocationPoints.first.timestamp}, Last point timestamp: ${_originalLocationPoints.last.timestamp}');
      AppLogger.debug(
          '[SESSION_EDITING] Cutoff duration: ${cutoffDuration.inSeconds}s, Points filtered out: ${_originalLocationPoints.length - filteredLocationPoints.length}');

      // Filter heart rate samples up to cutoff time
      final filteredHeartRateSamples = _originalHeartRateSamples
          .where((sample) =>
              sample.timestamp.isBefore(cutoffTime) ||
      ...
      final filteredSplits = _originalSplits
          .where((split) =>
              split.timestamp.isBefore(cutoffTime) ||
      ...
      print(
          '🔧 [SESSION_EDITING] Timeline position: $_timelinePosition (${(_timelinePosition * 100).toStringAsFixed(1)}%)');
      print(
          '🔧 [SESSION_EDITING] Original stored distance: ${originalDistance.toStringAsFixed(3)}km');
      print(
          '🔧 [SESSION_EDITING] Calculated distance from points: ${newDistance.toStringAsFixed(3)}km');
      print(
          '🔧 [SESSION_EDITING] Scaled distance (final): ${scaledDistance.toStringAsFixed(3)}km');
      print(
          '🔧 [SESSION_EDITING] Points filtered out: ${_originalLocationPoints.length - filteredLocationPoints.length}');
      print(
          '🔧 [SESSION_EDITING] Expected time reduction: ${(100 - (_timelinePosition * 100)).toStringAsFixed(1)}%');
      print(
          '🔧 [SESSION_EDITING] Actual distance reduction: ${((originalDistance - scaledDistance) / originalDistance * 100).toStringAsFixed(1)}%');
      print(
          '🔧 [SESSION_EDITING] Distance difference: ${(originalDistance - scaledDistance).toStringAsFixed(3)}km');
      print(
          '🔧 [SESSION_EDITING] Using original distance as baseline: ${(_timelinePosition >= 1.0) ? "YES" : "NO"}');

      AppLogger.debug(
          '[SESSION_EDITING] Calculated distance: ${newDistance.toStringAsFixed(3)}km from ${filteredLocationPoints.length} points');
      ...
      final newCalories =
          (widget.originalSession.caloriesBurned * _timelinePosition).round();
      print(
          ' [SESSION_EDITING] Calorie scaling: ${widget.originalSession.caloriesBurned} * $_timelinePosition = $newCalories');
      AppLogger.debug(
          '[SESSION_EDITING] Calories calculation: original=${widget.originalSession.caloriesBurned}, new=${newCalories}, cutoff duration=${cutoffDuration.inMinutes}min vs original=${totalDuration.inMinutes}min');

      AppLogger.debug(
          '[SESSION_EDITING] Timeline position: $_timelinePosition, Original duration: ${widget.originalSession.duration.inMinutes}min, Cutoff duration: ${cutoffDuration.inMinutes}min, Original calories: ${widget.originalSession.caloriesBurned}, New calories: $newCalories');

      // Calculate remaining metrics
      final newAveragePace = _calculateAveragePace(newDistance, cutoffDuration);
      final newHeartRateStats =
          _calculateHeartRateStats(filteredHeartRateSamples);

      // Debug preview session creation
      print('🔧 [SESSION_EDITING] === PREVIEW SESSION CREATION ===');
      print(
          '🔧 [SESSION_EDITING] About to create preview session with distance: ${scaledDistance.toStringAsFixed(3)}km');
      print(
          '🔧 [SESSION_EDITING] Original session distance: ${widget.originalSession.distance.toStringAsFixed(3)}km');
      ...
      print(
          '🔧 [SESSION_EDITING] Created preview session with distance: ${_previewSession!.distance.toStringAsFixed(3)}km');
      print(
          '🔧 [SESSION_EDITING] Preview session calories: ${_previewSession!.caloriesBurned}');

      AppLogger.debug(
          '[SESSION_EDITING] Updated preview: ${filteredLocationPoints.length} points, ${scaledDistance.toStringAsFixed(2)}km');
    } catch (e) {
      AppLogger.error('[SESSION_EDITING] Error calculating preview session',
          exception: e);
    }
  }
  ...
  int _calculateCalories(Duration duration, double ruckWeightKg) {
    ...
    return ((baseCaloriesPerHour + (weightFactor * 50)) * durationHours)
        .round();
  }
  ...
  Map<String, int?> _calculateHeartRateStats(List<HeartRateSample> samples) {
    ...
    return {
      'avg': totalHeartRate != null
          ? (totalHeartRate / samples.length).round()
          : null,
      'max': maxHeartRate,
      'min': minHeartRate,
    };
  ...
  void _saveEditedSession() async {
    ...
    try {
      ...
      AppLogger.info(
          '[SESSION_EDITING] Making API call to update session ${_previewSession!.id}');
      final sessionRepository = GetIt.instance<SessionRepository>();
      final updatedSession =
          await sessionRepository.updateSession(_previewSession!);

      // Update the preview session with the backend response
      _previewSession = updatedSession;
      AppLogger.info(
          '[SESSION_EDITING] Successfully updated session ${updatedSession.id}');
      ...
      AppLogger.error('[SESSION_EDITING] Error saving edited session',
          exception: e);
  ...
  Widget _buildRouteMap() {
    ...
    final cutoffIndex =
        (_originalLocationPoints.length * _timelinePosition).round();
    ...
    final visibleLatLng =
        visiblePoints.map((p) => LatLng(p.latitude, p.longitude)).toList();
    final hiddenLatLng =
        hiddenPoints.map((p) => LatLng(p.latitude, p.longitude)).toList();

    // Calculate map bounds
    final allLatLng = _originalLocationPoints
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      ...
                      _buildMetricRow(
                        'Distance',
                        MeasurementUtils.formatDistance(original.distance,
                            metric: preferMetric),
                      ...
                      _buildMetricRow(
                        'Pace',
                        MeasurementUtils.formatPace(original.averagePace,
                            metric: preferMetric),
                        Colors.grey.shade300,
                      ),
                      _buildMetricRow(
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      ...
                      _buildMetricRow(
                        'Distance',
                        MeasurementUtils.formatDistance(preview.distance,
                            metric: preferMetric),
                        AppColors.primary,
                      ),
                      _buildMetricRow(
                      ...
                      _buildMetricRow(
                        'Pace',
                        MeasurementUtils.formatPace(preview.averagePace,
                            metric: preferMetric),
                        AppColors.primary,
                      ),
                      _buildMetricRow(
