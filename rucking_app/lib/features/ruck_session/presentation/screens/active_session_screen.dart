...
class _ActiveSessionScreenState extends ConsumerState<ActiveSessionScreen> with WidgetsBindingObserver {
  ...
  void _calculateStats(LocationPoint newPoint) {
    AppLogger.info('[STATS] _calculateStats called with newPoint: lat=${newPoint.latitude}, lng=${newPoint.longitude}, time=${newPoint.timestamp}');
    ...
    if (_canShowStats) {
      ...
      final weightKg = _userWeightKg + (ruckWeightKg * 0.75);
      AppLogger.info('[STATS] segmentTimeMinutes: $segmentTimeMinutes, weightKg: $weightKg');
      // Calculate calories burned for this segment
      final segmentCalories = MetCalculator.calculateCaloriesBurned(
        weightKg: weightKg, // Count 75% of ruck weight
        durationMinutes: segmentTimeMinutes, // Use fractional minutes
        metValue: metValue,
      );
      AppLogger.info('[STATS] segmentCalories: $segmentCalories');
      
      _caloriesBurned += segmentCalories;
      AppLogger.info('[STATS] _caloriesBurned (running total): $_caloriesBurned');
  ...
  Future<void> _startSession() async {
    // Set last location update time to now to prevent immediate auto-pause
