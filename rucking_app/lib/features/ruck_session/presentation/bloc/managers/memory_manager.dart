...
class MemoryManager implements SessionManager {
  ...
  static const Duration _autoSaveInterval =
      Duration(seconds: 30); // More frequent saves
  ...
  Future<void> _onMemoryUpdated(MemoryUpdated event) async {
    ...
    final shouldSaveImmediately =
        event.immediate || criticalKeys.contains(event.key);
    ...
    AppLogger.debug(
        '[MEMORY_MANAGER] Memory updated: ${event.key} (immediate: $shouldSaveImmediately)');
  }
