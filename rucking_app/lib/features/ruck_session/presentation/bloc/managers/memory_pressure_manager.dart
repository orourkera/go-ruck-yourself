...
class MemoryPressureManager implements SessionManager {
  ...
  void _checkMemoryPressure() {
    try {
      ...
      if (memoryUsageMb > 500.0) {
        ...
        AppLogger.critical('CRITICAL MEMORY USAGE', exception: {
        ...
        AppLogger.warning('High memory pressure detected: memory_usage_mb=${memoryUsageMb.toStringAsFixed(1)}, session_id=$_activeSessionId');
  ...
  void _adjustLocationTrackingForMemoryPressure() {
    try {
      ...
      _locationService.adjustTrackingFrequency(location_service.LocationTrackingMode.powerSave);
  ...
  location_service.LocationTrackingMode _convertToLocationServiceMode(LocationTrackingMode mode) {
    switch (mode) {
  ...
  void _adjustLocationTrackingMode(LocationTrackingMode targetMode) {
    try {
      ...
      if (_lastLocationMode != targetMode) {
        ...
        _locationService.adjustTrackingFrequency(_convertToLocationServiceMode(targetMode));
        _lastLocationMode = targetMode;
