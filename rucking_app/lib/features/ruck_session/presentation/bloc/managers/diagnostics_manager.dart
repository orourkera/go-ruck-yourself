...
class DiagnosticsManager implements SessionManager {
  ...
  void _reportSessionDiagnostics({bool isFinalReport = false}) {
    ...
    if (sessionDurationMinutes == 0 && !isFinalReport) return; // Avoid division by zero
    
    // Get memory usage information
    final memoryInfo = _getMemoryInfo();
    
    // Calculate rates and quality metrics
    final locationUpdatesPerMinute = sessionDurationMinutes > 0 ? (_locationUpdatesCount / sessionDurationMinutes) : 0.0;
    final heartRateUpdatesPerMinute = sessionDurationMinutes > 0 ? (_heartRateUpdatesCount / sessionDurationMinutes) : 0.0;
    ...
    final reportType = isFinalReport ? 'Final Session Report' : 'Session Performance Report';
    AppLogger.critical(reportType, exception: {
  ...
  Map<String, dynamic> get performanceMetrics => {
    ...
    'avg_api_latency_ms': _apiCallsCount > 0 ? (_totalApiLatencyMs / _apiCallsCount) : 0.0,
    'memory_usage_mb': memoryUsageMb,
  };
}
