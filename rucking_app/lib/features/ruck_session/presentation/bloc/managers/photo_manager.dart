...
class PhotoManager implements SessionManager {
  ...
  Future<void> _onPhotoAdded(PhotoAdded event) async {
    if (_activeSessionId == null) {
      AppLogger.warning(
          '[PHOTO_MANAGER] Cannot add photo without active session');
      return;
    }
  ...
  Future<void> _onPhotoDeleted(PhotoDeleted event) async {
    ...
    try {
      ...
      if (!_activeSessionId!.startsWith('offline_')) {
        AppLogger.info(
            '[PHOTO_MANAGER] Photo deletion not yet implemented on backend');
      }
  ...
  Future<void> _loadPhotos() async {
    ...
    try {
      final photos =
          await _sessionRepository.getSessionPhotos(_activeSessionId!);
      _photos.clear();
  ...
  Future<void> _savePhotoLocally(RuckPhoto photo) async {
    try {
      ...
      final existingPhotos = existingPhotosJson != null
          ? List<Map<String, dynamic>>.from(jsonDecode(existingPhotosJson))
          : [];
  ...
  Future<void> _uploadPhoto(RuckPhoto photo) async {
    try {
      if (_isUploading || photo.url == null || !photo.url!.startsWith('/'))
        return;
      ...
      try {
        AppLogger.info(
            '[PHOTO_MANAGER] Uploading photo to backend: ${photo.filename}');
        ...
        final uploadedPhotos =
            await _sessionRepository.uploadSessionPhotosOptimized(
          photo.ruckId,
        ...
        if (uploadedPhotos.isNotEmpty) {
          ...
          AppLogger.info(
              '[PHOTO_MANAGER] Photo uploaded successfully: $uploadedPhotoUrl');
        } else {
  ...
  Future<void> takePhoto() async {
    if (_activeSessionId == null) {
      AppLogger.warning(
          '[PHOTO_MANAGER] Cannot take photo without active session');
      return;
    }
  ...
  Future<void> selectPhoto() async {
    if (_activeSessionId == null) {
      AppLogger.warning(
          '[PHOTO_MANAGER] Cannot select photo without active session');
      return;
    }
