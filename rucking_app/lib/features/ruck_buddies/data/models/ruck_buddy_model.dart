        ...
        if (photosData is List) {
          ...
          if (photoList.isNotEmpty) {
            if (photoList.first is Map) {
              photos = photoList.map((item) => RuckPhoto.fromJson(item as Map<String, dynamic>)).toList();
            } else if (photoList.first is String) {
              photos = photoList.map((url) => RuckPhoto(
              ...
              for (final item in photoList) {
                try {
                  if (item is Map) {
                    photos.add(RuckPhoto.fromJson(item as Map<String, dynamic>));
          ...
          try {
            ...
            if (decodedPhotos is List) {
              photos = decodedPhotos.map((item) => RuckPhoto.fromJson(item as Map<String, dynamic>)).toList();
            }
          } catch (jsonErr) {
            print('Error decoding photos JSON string: $jsonErr');
            if (photosData.contains(',')) {
              final urlList = photosData.split(',').map((url) => url.trim()).where((url) => url.isNotEmpty).toList();
              photos = urlList.map((url) => RuckPhoto(
