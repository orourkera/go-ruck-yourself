...
class AchievementRepositoryImpl implements AchievementRepository {
  ...
  Future<List<Achievement>> getAllAchievements({String? unitPreference}) async {
    try {
      ...
      if (cachedData != null) {
        debugPrint('🏆 [AchievementRepository] Found ${cachedData.length} achievements in cache');
      ...
      if (response['status'] == 'success' && response['achievements'] != null) {
        ...
        debugPrint('🏆 [AchievementRepository] Cached ${achievementsData.length} achievements');
  ...
  Future<List<UserAchievement>> getUserAchievements(String userId) async {
    try {
      ...
      if (cachedData != null) {
        debugPrint('🏆 [AchievementRepository] Found ${cachedData.length} user achievements in cache');
        return cachedData.map((json) => UserAchievement.fromJson(json)).toList();
      }
      
      debugPrint('🏆 [AchievementRepository] Cache miss, fetching user achievements from API');
      
      final endpoint = ApiEndpoints.getUserAchievementsEndpoint(userId);
      debugPrint('🏆 [AchievementRepository] API endpoint: $endpoint');
      
      final response = await _apiClient.get(endpoint);
      debugPrint('🏆 [AchievementRepository] User achievements response: $response');
      
      if (response['status'] == 'success' && response['user_achievements'] != null) {
        final userAchievementsData = response['user_achievements'] as List;
        debugPrint('🏆 [AchievementRepository] Found ${userAchievementsData.length} user achievements');
        
        // Cache the data
        await _cacheService.cacheUserAchievements(userId, userAchievementsData);
        debugPrint('🏆 [AchievementRepository] Cached ${userAchievementsData.length} user achievements');
      ...
      debugPrint('🏆 [AchievementRepository] No user achievements found or invalid response');
      return [];
    } catch (e) {
  ...
  Future<AchievementStats> getAchievementStats(String userId, {String? unitPreference}) async {
    try {
      ...
      if (cachedData != null) {
        print('[DEBUG] AchievementRepository: Found achievement stats in cache');
        return AchievementStats.fromJson(cachedData);
      }
      
      print('[DEBUG] AchievementRepository: Cache miss, fetching stats from API');
  ...
  Future<List<UserAchievement>> getRecentAchievements() async {
    try {
      ...
      if (cachedData != null) {
        debugPrint('🏆 [AchievementRepository] Found ${cachedData.length} recent achievements in cache');
        return cachedData.map((json) => UserAchievement.fromJson(json)).toList();
      }
      
      debugPrint('🏆 [AchievementRepository] Cache miss, fetching recent achievements from API');
      ...
      if (response['status'] == 'success' && response['recent_achievements'] != null) {
        ...
        debugPrint('🏆 [AchievementRepository] Cached ${recentAchievementsData.length} recent achievements');
