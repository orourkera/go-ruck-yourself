...
class NotificationRemoteDataSourceImpl implements NotificationRemoteDataSource {
  ...
  Future<List<NotificationModel>> getNotifications() async {
    try {
      ...
      AppLogger.info(
          'Notifications API response received: ${response.runtimeType}');
      ...
      if (response is Map<String, dynamic> &&
        ...
        AppLogger.info(
            'Found ${notificationsData.length} notifications in response');
      } else if (response is List) {
        notificationsData = response;
        AppLogger.info(
            'Response is direct list with ${notificationsData.length} notifications');
      } else {
        AppLogger.warning(
            'Unexpected notifications response format: ${response.runtimeType}');
      ...
      AppLogger.info(
          'Successfully parsed ${notifications.length} notifications (${unreadCount} unread)');
  ...
  Future<bool> markNotificationAsRead(String notificationId) async {
    try {
      ...
      AppLogger.info(
          'Marking notification as read: $notificationId at endpoint: $endpoint');
  ...
  Future<bool> markAllNotificationsAsRead() async {
    try {
      ...
      AppLogger.info(
          'Marking all notifications as read at endpoint: $endpoint');
      final response = await apiClient.post(endpoint, {});
