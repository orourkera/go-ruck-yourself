...
- (void)updateSessionOnWatchDistance:(double)arg_distance duration:(double)arg_duration pace:(double)arg_pace isPaused:(BOOL)arg_isPaused calories:(double)arg_calories elevationGain:(double)arg_elevationGain elevationLoss:(double)arg_elevationLoss completion:(void (^)(NSNumber *_Nullable, FlutterError *_Nullable))completion {
  ...
  FlutterBasicMessageChannel *channel =
    [FlutterBasicMessageChannel
      ...
      binaryMessenger:self.binaryMessenger
      codec:nullGetRuckingApiCodec()];
  [channel sendMessage:@[@(arg_distance), @(arg_duration), @(arg_pace), @(arg_isPaused), @(arg_calories), @(arg_elevationGain), @(arg_elevationLoss)] reply:^(NSArray<id> *reply) {
    if (reply != nil) {
      if (reply.count > 1) {
        completion(nil, [FlutterError errorWithCode:reply[0] message:reply[1] details:reply[2]]);
      } else {
  ...
  /// Update session metrics on the watch
  {
    FlutterBasicMessageChannel *channel =
      [[FlutterBasicMessageChannel alloc]
        ...
        codec:nullGetRuckingApiCodec()];
    if (api) {
      NSCAssert([api respondsToSelector:@selector(updateSessionOnWatchDistance:duration:pace:isPaused:calories:elevationGain:elevationLoss:error:)], @"FlutterRuckingApi api (%@) doesn't respond to @selector(updateSessionOnWatchDistance:duration:pace:isPaused:calories:elevationGain:elevationLoss:error:)", api);
      [channel setMessageHandler:^(id _Nullable message, FlutterReply callback) {
        ...
        double arg_calories = [GetNullableObjectAtIndex(args, 4) doubleValue];
        double arg_elevationGain = [GetNullableObjectAtIndex(args, 5) doubleValue];
        double arg_elevationLoss = [GetNullableObjectAtIndex(args, 6) doubleValue];
        FlutterError *error;
        [api updateSessionOnWatchDistance:arg_distance duration:arg_duration pace:arg_pace isPaused:arg_isPaused calories:arg_calories elevationGain:arg_elevationGain elevationLoss:arg_elevationLoss error:&error];
