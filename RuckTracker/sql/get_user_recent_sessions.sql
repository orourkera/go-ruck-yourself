...
BEGIN
    ...
    FOR session_record IN
        SELECT 
            ...
            rs.status,
            rs.is_manual,
            -- Social counts for user's own sessions
        ...
        LIMIT p_limit
    LOOP
        -- For user's own sessions, they get full route data (no privacy clipping needed)
        -- But still use intelligent sampling to reduce memory usage
        SELECT json_agg(
            json_build_object(
                'latitude', latitude,
                'longitude', longitude,
        ...
        FROM get_sampled_route_points(
            session_record.id,
            -- Sampling interval based on distance (same logic as public sessions)
            CASE
                ...
                ELSE 12
            END,
            -- Max points based on distance (same logic as public sessions)
            CASE 
