-- Migration: Fix last_ruck_weight_kg returning 0 instead of actual weight
-- Author: Assistant 2025-09-28
-- Purpose: Fix the logic for retrieving the last ruck weight from sessions
...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM "user" WHERE id = u_id) THEN
    RETURN jsonb_build_object(
      ...
      'recency', jsonb_build_object('last_completed_at', null, 'days_since_last', null, 'last_ruck_weight_kg', 0),
      'recent_splits', '[]'::jsonb,
  ...
  -- Get the last ruck weight separately
  SELECT ruck_weight_kg
  INTO last_weight
  FROM ruck_session
  WHERE user_id = u_id
    AND status = 'completed'
    AND COALESCE(duration_seconds, 0) >= 300
    AND ruck_weight_kg IS NOT NULL
    AND ruck_weight_kg > 0
  ...
  BEGIN
    ...
    SELECT jsonb_build_object(
      ...
      'recency', jsonb_build_object(
        ...
        'last_ruck_weight_kg', COALESCE(last_weight, (SELECT ruck_weight_kg FROM last_done), 0)
      ),
    ...
    INTO temp_facts
    FROM span30, span90, all_time, load_stats;
  ...
  BEGIN
    WITH recent_sessions AS (
      SELECT id, completed_at, ruck_weight_kg
      FROM ruck_session
      WHERE user_id = u_id AND status = 'completed'
        AND COALESCE(duration_seconds, 0) >= 300
      ORDER BY completed_at DESC
    ...
    last3 AS (
      SELECT id, completed_at, ruck_weight_kg FROM recent_sessions ORDER BY completed_at DESC LIMIT 3
    ),
    sample AS (
      SELECT
        s.session_id,
